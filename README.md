...
Time-Block Hero 的世界观展示站与本地设计工具仓库。网站由游戏设定集、规则 Wiki 和卡牌编辑器组成，所有入口均无需密码。规则 Wiki 已按确认的大纲分为通用游戏规则、标准对战、关键词库与裁定，并附导读和维护记录；规则正文仍待逐项确认。第七章提供四类效果设计模板与八类共享字典，可从规则 Wiki 的分层目录进入；源码与写作约定见 [规则 Wiki 交接](docs/rules/README.md)。
