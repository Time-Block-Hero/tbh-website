...
(() => {
  ...
  if (!Array.isArray(bestiaryRecords) || !bestiaryRecords.length) {
    root.textContent = isEnglish
      ? "The creature archive could not be loaded."
      : "生物档案暂时无法加载。";
    return;
  }
  const copy = {
    ...
    recordCount: (count) =>
      isEnglish ? `${count} records found` : `检索到 ${count} 份档案`,
    ...
    knownSpecimens: isEnglish
      ? "Known specimens and prior artwork"
      : "已知个体与既有插画",
  ...
  const searchIndex = new Map(
    bestiaryRecords.map((record) => [record.id, searchableText(record)]),
  );
  ...
  let activeRecord =
    bestiaryRecords.find((record) => record.id === initialId) ||
    bestiaryRecords[0];
  const lightbox = document.createElement("dialog");
  lightbox.className = "creature-lightbox";
  lightbox.setAttribute(
    "aria-label",
    isEnglish ? "Full image preview" : "原图预览",
  );
  ...
  function visibleRecords() {
    ...
    return bestiaryRecords.filter(
      (record) =>
        (activeFilter === "all" || record.group === activeFilter) &&
        (!query || searchIndex.get(record.id).includes(query)),
    );
  }
  ...
  function renderFilters() {
    filterEl.innerHTML = filters
      .map(
        (filter) =>
          `<button type="button" data-filter="${filter}" aria-pressed="${activeFilter === filter}">${escapeHtml(copy[filter])}</button>`,
      )
      .join("");
  }
  function renderList() {
    ...
    if (
      !records.some((record) => record.id === activeRecord.id) &&
      records.length
    )
    ...
    listEl.innerHTML = records
      .map(
        (record) =>
          `<button type="button" class="creature-index-item" data-record="${escapeHtml(record.id)}" aria-pressed="${record.id === activeRecord.id}"><img src="${escapeHtml(record.illustration)}" alt="" loading="lazy"><span><small>${String(bestiaryRecords.indexOf(record) + 1).padStart(2, "0")} / ${escapeHtml(copy[record.group])}</small><strong>${escapeHtml(text(record, "zhName", "enName"))}</strong><em>${escapeHtml(text(record, "zhSubtitle", "enSubtitle"))}</em></span></button>`,
      )
      .join("");
    renderDetail();
  }
  ...
  function renderDetail() {
    ...
    const knownFormsNote =
      text(record, "zhKnownFormsNote", "enKnownFormsNote") ||
      copy.knownFormsNote;
    const protocol = isEnglish
      ? record.protocolEn || []
      : record.protocolZh || [];
    const conceptNote =
      text(record, "zhConceptNote", "enConceptNote") || copy.pendingDefault;
    ...
    <div class="bestiary-body">
      <div class="bestiary-facts">
        ${[
          ...
          .map(
            ([label, value]) =>
              `<div class="bestiary-fact"><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong></div>`,
          )
          .join("")}
      </div>
  ...
  function selectRecord(id) {
    ...
    listEl
      ...
      .forEach((button) =>
        button.setAttribute(
          "aria-pressed",
          String(button.dataset.record === id),
        ),
      );
    renderDetail();
  ...
  root.addEventListener(
    ...
    (event) => {
      ...
      placeholder.textContent = isEnglish
        ? "Image unavailable · awaiting replacement"
        : "插画文件缺失 · 待补全";
      const button = image.closest("[data-lightbox-src]");
      if (button) {
        button.disabled = true;
        button.removeAttribute("data-lightbox-src");
      }
