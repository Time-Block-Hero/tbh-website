...
import {
  buildSettingData,
  validateSettingAssets,
} from "../build-setting-data.mjs";
...
const config = JSON.parse(
  fs.readFileSync(path.join(root, "data/setting.json"), "utf8"),
);
const cards = JSON.parse(
  fs.readFileSync(path.join(root, "data/cards.json"), "utf8"),
);
...
test("current dossiers preserve named people and distinct forms without old placeholder identities", () => {
  ...
  assert.equal(
    data.characters.find((item) => item.id === "vera").name,
    "铁驭姬-维拉",
  );
  assert.equal(
    data.characters.find((item) => item.id === "drake").sources.length,
    2,
  );
  assert.equal(
    data.characters.find((item) => item.id === "jinbai").sources.length,
    2,
  );
...
test("missing, renamed, duplicated, or repurposed identities fail instead of falling back to IDs", () => {
  ...
  missing.cards = missing.cards.filter(
    (card) => card.artworkKey !== "titan-maiden-vera",
  );
  assert.throws(
    () => buildSettingData(config, missing),
    /Missing source identity/,
  );
  const renamed = structuredClone(cards);
  renamed.cards.find(
    (card) => card.artworkKey === "titan-maiden-vera",
  ).artworkKey = "new-vera-key";
  assert.throws(
    () => buildSettingData(config, renamed),
    /Missing source identity/,
  );
  const duplicate = structuredClone(cards);
  duplicate.cards.push({
    ...duplicate.cards.find((card) => card.artworkKey === "titan-maiden-vera"),
    id: "DUPLICATE",
  });
  assert.throws(
    () => buildSettingData(config, duplicate),
    /Ambiguous artworkKey/,
  );
  const repurposed = structuredClone(cards);
  repurposed.cards.find(
    (card) => card.artworkKey === "titan-maiden-vera",
  ).nameKey = "无关角色";
  assert.throws(
    () => buildSettingData(config, repurposed),
...
test("broken selected artwork does not silently promote a candidate", () => {
  ...
  const vera = broken.cards.find(
    (card) => card.artworkKey === "titan-maiden-vera",
  );
  broken.selectedArtworkIds[vera.id] = "missing-selection";
  assert.throws(
    () => buildSettingData(config, broken),
    /Missing selected artwork/,
  );
});
...
const summaries = JSON.parse(
  fs.readFileSync(path.join(root, "data/character-summaries.json"), "utf8"),
);

test("visitor introductions are source-bound and retired after an identity or art brief change", () => {
  const before = buildSettingData(config, cards, summaries);
  assert.equal(
    before.characters.filter((entry) => entry.appearanceSummary).length,
    22,
  );
  const changed = structuredClone(cards);
  changed.cards.find(
    (card) => card.artworkKey === "titan-maiden-vera",
  ).artDescription = "New unreviewed design";
  const after = buildSettingData(config, changed, summaries);
  assert.equal(
    after.characters.find((entry) => entry.id === "vera").appearanceSummary,
    null,
  );
  assert.ok(
    after.characters.find((entry) => entry.id === "mira").appearanceSummary,
  );
});

test("reordering numeric card IDs does not invalidate visitor introductions", () => {
  const changed = structuredClone(cards);
  const card = changed.cards.find(
    (card) => card.artworkKey === "titan-maiden-vera",
  );
  const oldId = card.id;
  card.id = "TEST-NEW-ID";
  changed.artworkVariants[card.id] = changed.artworkVariants[oldId];
  changed.selectedArtworkIds[card.id] = changed.selectedArtworkIds[oldId];
  assert.deepEqual(
    buildSettingData(config, changed, summaries).characters.find(
      (entry) => entry.id === "vera",
    ).appearanceSummary,
    { zh: summaries.vera.zh, en: summaries.vera.en },
  );
});
