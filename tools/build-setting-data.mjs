...
export function buildSettingData(config, cardData) {
  if (config.schemaVersion !== 1) throw new Error("Unsupported setting schema version");
  const byKey = new Map();
  for (const card of cardData.cards) {
    if (!card.artworkKey) continue;
    if (byKey.has(card.artworkKey)) throw new Error(`Ambiguous artworkKey: ${card.artworkKey}`);
    byKey.set(card.artworkKey, card);
  }
  const resolve = (key, identityNameIncludes) => {
    const card = byKey.get(key);
    if (!card) throw new Error(`Missing source identity: ${key}; update the explicit dossier mapping after a rename`);
    if (identityNameIncludes && !card.nameKey.includes(identityNameIncludes)) {
      throw new Error(`Source identity mismatch: ${key} no longer names ${identityNameIncludes}`);
    }
    const selectedId = cardData.selectedArtworkIds?.[card.id];
    const selected = (cardData.artworkVariants?.[card.id] || []).find((item) => item.id === selectedId);
    if (selectedId && !selected) throw new Error(`Missing selected artwork: ${card.id} / ${selectedId}`);
    return {
      cardId: card.id,
      artworkKey: key,
      name: card.nameKey,
  ...
  if (factions.length !== 4 || new Set(factions.map((item) => item.classId)).size !== 4) {
    throw new Error("Exactly four distinct non-neutral class concepts are required");
  }
  const factionByClass = new Map(factions.map((faction) => [faction.classId, faction.id]));
  const seen = new Set();
  const characters = config.characters.map((entry) => {
    ...
    seen.add(entry.id);
    const sources = entry.sourceArtworkKeys.map((key) => resolve(key, entry.identityNameIncludes));
    const primary = sources[0];
    if (!primary && !entry.retainedIdentity) throw new Error(`Dossier ${entry.id} has no approved identity source`);
    if (primary && sources.some((source) => source.classId !== primary.classId)) {
      throw new Error(`Dossier ${entry.id} has conflicting classes; review the identity map`);
    }
    ...
    return {
      ...
      sources,
      sourceNote: primary ? "人物身份与美术描述来自最新卡牌设计；完整档案尚未完成。" : entry.retainedIdentity.reason,
  ...
  return { schemaVersion: 1, background: config.background, factions, characters };
}

export function validateSettingAssets(data, directory = root) {
  const images = [
    ...data.factions.map((faction) => faction.image),
    ...data.characters.flatMap((character) => character.sources.map((source) => source.cover)),
  ].filter(Boolean);
...
export function buildSettingFile(directory = root) {
  const config = JSON.parse(fs.readFileSync(path.join(directory, "data/setting.json"), "utf8"));
  const cards = JSON.parse(fs.readFileSync(path.join(directory, "data/cards.json"), "utf8"));
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  console.log(`Built setting-data.js: ${data.characters.length} character dossiers, ${data.factions.length} class concepts.`);
}
