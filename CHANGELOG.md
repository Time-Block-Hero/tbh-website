...
- 同步14张卡牌的最新平衡设计：费用、属性、箭头及种族调整；阳炎信徒改保留光环、俱焚烈焰改主动发动、训练服务机群奖励改由自身生成星能、星能循环模块进度目标改为6。稳定UID与收集准入不变。

- Issue44：蜂群无人机改为召唤复制当前属性的独立幻影（耐久1）；运输管线完成奖励明确为快速；学院明确电力仅加速进度，每次完成恒定降低1点建造目标。

- 明确填满进度与直接触发的区别：借时蜉蝣、集群主脑补至有效目标并正常完成，循环归零，一次性不重复；集群主脑卡文补明随机选取。

- Issue44：固定猎空同盟、烈阳教会与中立修订的最新编辑器数据；历史空白卡现在允许设计师补充效果，导出记录当前空白集合而非永久禁止编辑。

- 固定2026-09-20网页改卡快照：132张设计；统一点数分配、慢速入场准备和真实保留规则，拆分设计转译与引擎执行验收。

- 明确海燕穿越中断时的后备落点：此前路径全部不可落脚时，允许修正到中断位置最近的合法空格；不退还或额外消耗行动点。

- 入场效果默认在独立时停结束后结算，新增快速关键词；目标在窗口内准备，队列按确认提交先后执行，待结算信息遵守隐藏信息规则。

### Card design identity and export

- 为137张现行设计保存稳定UID、显式衍生关系及四类收集身份；编辑器支持资源类型，七张既定资源完成类型迁移，重排和改名不再改变内部身份。
- 新增提交SHA与双文件哈希验证的纯设计导出；旧formal参考改为明确的开发预览，不再继承历史效果和支持标记。保留四项测试排除、四项空白设计及重复同名牌的独立身份。
