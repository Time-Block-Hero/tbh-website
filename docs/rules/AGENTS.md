...
- Give each rule one canonical home and link to it elsewhere. Keep placement examples under placement, movement under movement, operation-window definitions in chapter 4, and keyword lookup entries in the keyword library. When the user assigns a detailed mechanic to an action section (for example, construction in 4.2), keep its definition there and link from the keyword entry. Keep card-effect design vocabulary in chapter 7 and cross-cutting settlement timing in chapter 8.
- Model chapter 7 as independent activated abilities and continuous effects that provide triggered, replacement, stat, value, movement and placement contributions. Execution content is separate. Verify the pinned runtime content structure, but do not promote historical implementation or architecture proposals over confirmed gameplay decisions.
- Present dictionaries as compact comparison tables with stable row anchors, not one heading and vertical table per term. Keep essential meaning, parameters and examples visible; use supported Markdown detail blocks only for extended boundaries. Preserve term search and links without crowding the table of contents.
