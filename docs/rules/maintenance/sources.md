...
- 用户根据试玩明确将每方领土由6×6缩小为 **5×5**。配套采用原布局的外围一格、中央两行，因而整盘改为 **14行×7列**；A领土第9–13行、B领土第2–6行，均为第2–6列。整盘尺寸与坐标是本次编辑的配套默认值，不伪称用户亲自指定了14×7。英雄部署同步缩为最后一排5个可选格。 此处14×7布局已由[去除领地后方空行的明确决定](#标准地图去除领地后方空行)替代，保留为历史记录。
...
下文保留当时的迁移记录；其中仅以入场描述默认速度、将其他普通触发与之分开的限制性表述，已由[普通触发默认慢速的适用范围](#普通触发默认慢速的适用范围2026-09-21)纠正。
...
## 普通触发默认慢速的适用范围（2026-09-21）

用户在 Issue 44 本批卡牌导入中明确：默认慢速适用于所有普通卡牌触发效果，包括移除、被保留等；显式 `[快速]` 的具体效果即时执行。此前文档将默认范围误限为入场、将其他普通触发解释为即时，本次予以纠正，不再用该旧表述限制职业卡转录。

触发条件满足的时点与正文执行的速度分开。发生前响应、替换、持续生效贡献不属于普通触发的默认延后；明确指定的湮灭阶段、拾取等专门时序保持。慢速只在双方独立时停期间延后，时停之外及阶段末结算期间的新后续按正常顺序当场完成。玩家选择快速准备、随机默认留到正文执行、已确认提前随机特例及服务器确认登记顺序均保持。

主定义：[3.3.2](../common/rounds.md#待结算效果的登记与顺序)；依赖页已同步更新第四、六、七、八章及关键词。规则确认与引擎/卡牌实现验收仍分别记录在[Issue 44](https://github.com/Time-Block-Hero/time-block-hero-unified/issues/44)。


## 填满进度的实际完成语义（2026-09-22）

用户在 Issue 44 剩余职业导入的具体问答中确认：集群主脑-埃米莉丝（MCC-005）与借时蜉蝣（MCC-010）采用当前卡面的“填满”语义，集群主脑继续随机选三个。填满将当前值补至当前有效目标，然后正常登记完成；循环进度扣除目标后归0，一次性进度标记完成，已完成一次性不因填满重复执行。本次裁定覆盖此前来源与第28组历史记录中“这两张卡直接触发并保留原进度”的解释；历史记录作为旧决定存档，不再指导当前两卡。

两张卡的“卡牌”措辞保持：集群主脑随机取不同的合法卡牌，不按同一张卡上的进度项重复计算名额；借时蜉蝣按卡面方向作用于格内合法卡牌。所选卡牌的有效未完成进度按填满规则处理；效果当前被建造或沉默等封锁时，不借填满绕过资格。MCC-005的卡文仅补入“随机”，没有改其30格目标、数量、成本或属性；MCC-010卡文不改。具体执行主定义为[6.2.6](../common/lifecycle.md#进度)，第七章、关键词及审阅索引同步引用。

明确写“直接触发”的通则继续保留原进度与一次性标记，作为不同操作，不因本轮填满裁定被删除。保存规则与卡文不等于引擎验收通过，执行回执另见[unified #44](https://github.com/Time-Block-Hero/time-block-hero-unified/issues/44)。

## 标准地图去除领地后方空行

用户在 Issue 44 试玩调整中明确要求删除双方领地背后的两行空行，即上、下各一行。标准地图由14×7改为12×7；双方5×5领土、左右各一列无归属边列和中央两行保留。固定俯视坐标中，B领土为第1–5行、A领土为第8–12行，均占第2–6列；中央无归属区域为第6–7行。英雄部署仍由玩家在己方领土最后一排选择，B为第1行、A为第12行。

这项明确决定替代此前配套默认布局中的上下空行，不改领土大小、中央间隔或卡牌设计。当前主定义见[10.3 地图与英雄部署](../standard/setup.md#地图与英雄部署)；引擎和场景同步由[unified #44](https://github.com/Time-Block-Hero/time-block-hero-unified/issues/44)跟进。
