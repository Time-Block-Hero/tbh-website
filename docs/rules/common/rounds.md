...
冻结不使原本隐藏的卡牌身份变成公开信息。放置、移除、保留等实际变化及其触发记录按各自规则发生；普通卡牌触发的正文默认慢速，未标明[快速](../reference/keywords.md#快速)时先完成规定的准备，再登记到本次时停结束的队列。触发条件已满足不代表正文立即执行，完整速度规则见[3.3.2](#待结算效果的登记与顺序)。发生前响应、替换与持续生效的贡献按自身时点处理，不放入普通慢速队列。独立时停中不提供反制对手本次新发动效果的能力。资源拾取仍进入下述专门步骤，不在独立部署期间提前取得收益。
...
**普通卡牌触发默认慢速（已确认）。** 独立时停期间，所有未标明[快速](../reference/keywords.md#快速)的普通卡牌触发效果，正文均登记到本次时停末；不只限于入场，也包括移除、被保留、死亡、阶段开始、进度完成等普通触发。卡文中的“移除时”“被保留时”等决定何时满足触发条件，不自行赋予快速速度。显式快速只让它修饰的那份正文当场执行，不使同一张牌或该正文引出的其他普通触发一并加速。

**响应、持续及专门时序分开处理。** 发生前响应、替换以及持续生效的属性等贡献，必须在对应检查点生效，不属于上述普通触发的默认延后。科学纪元驱魔师等已明确采用湮灭阶段的专门效果，按其明确时点处理；拾取仍有专门步骤。慢速只延后独立时停期间产生的正文；时停以外，以及时停末已经在处理湮灭、拾取、慢速时新产生的普通后续，当场按正常结算顺序完成，不再等待下一次时停。

**先准备，再登记。** 普通触发效果满足条件后，在效果所属玩家的操作窗口中完成当前可以确定且必须预先指定的目标、模式或分配，以及适用的发动成本，然后登记正文。依赖正文执行结果才能产生的选择，留到对应步骤；除矿石扫描等已明确提前随机的特例外，卡文明示的随机选取与分配同样留在正文执行时，以当时的合法候选进行，不在慢速效果准备时预抽或锁定候选。对手本回合新放置且届时仍合法的单位可以被选中，详见[8.2.1](effects.md#选择时点与操作窗口)。本人准备尚未完成时，对手仍可继续独立操作。
