...
5. **结算本次时停中产生的慢速效果（已确认）。** 湮灭及其应处理的后续结束后，处理这些慢速效果及其后续；全部完成之前，不进入任何玩家的时动阶段。
...
2. 使在本回合结束到期的临时效果和状态失效，并处理明确到期的形态还原；**“本回合获得保留”的到期延后至步骤 5**，不能在本次弃牌前先将其清除。
...
5. **临时保留先保护本次弃牌，再到期（已确认）。** 本次凭“本回合获得保留”免于弃牌的手牌，先按[6.2.7](lifecycle.md#被保留的回合数)更新保留次数，再使这份临时保留到期。到期不追加一次弃牌，也不撤销本次保留计数。
6. 清理和后续结算全部完成后，回合号增加，进入下一回合开始。
...
**场上清理依据[卡牌类型](elements.md#卡牌种类与分类维度)。** 步骤 3 不扩展为清除所有场上卡牌；[收集属性](elements.md#收集属性)也不另加清理规则。

本次实际被保留的牌，按[6.2.7 保留回合数](lifecycle.md#被保留的回合数)更新状态；这与单纯因不在弃牌快照而留在手牌的情况应分别判断，特殊边界见 LIF-R11。

星能处理见[5.1](economy.md#星能)，下一回合的额度刷新见[3.2](#回合开始)。快照后的状态变化和同批清理边界集中于[审阅事项](../maintenance/pending.md#第六章审阅事项)。
