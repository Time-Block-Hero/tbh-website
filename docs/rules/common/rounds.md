...
4. **处理抽牌引发的效果。** 本批抽取完成后处理相关效果。此时双方都没有操作窗口；任何原本要求玩家选择的步骤，按[4.1.1 操作窗口](actions.md#操作窗口)自动选择，不暂停等待玩家输入。
...
冻结不使原本隐藏的卡牌身份变成公开信息，也不将本方放置和入场效果推迟到时动才发生。**不设计在独立时停中反制对手新发动效果的能力**；「科学纪元驱魔师」原有时停反制效果已撤下。资源拾取统一进入下述时停结束流程，不在独立部署期间提前取得收益。
...
3. 应湮灭的牌直接进入各自弃牌堆，完成湮灭相关后续效果。双方均免湮灭不产生普通卡牌同格共存的例外。
...
双方的操作窗口此时均已关闭，慢速结算中的选择按窗口外规则处理。多项慢速效果的相互排序、在这个结算期间新产生慢速事项的处理，以及时停之外产生的慢速效果执行时点，仍需裁定；不默认改为来源所属玩家的时动开始。
...
本版本不提供额外时动阶段；「空亡时主」的额外时动奖励已撤下，后续版本如重新设计，须另行规定完整阶段流程。
...
此流程将手牌与法术放在同一快照和清理批次，不再先逐张清理法术再另建手牌清单。涉及替换而离区重入等特殊目标关系，仍按[8.2.3](effects.md#无合法选项与目标失效)判断，不重复执行已完成的清理。星能与下一回合额度分别见[5.1](economy.md#星能)、[3.2](#回合开始)。
