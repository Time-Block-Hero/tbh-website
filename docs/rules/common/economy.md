...
“下一次购买减费”只修正符合其条件的购买，不降低任意星能支付。护甲吸收伤害也不从玩家星能中扣除。

## 5.2 额外卡组与商店候选

**ECO-002 默认商店牌源。** 每位玩家的普通商店以自己的额外卡组为牌源。进入[同步购买阶段](rounds.md#购买)时，为各自商店自动产生最多 **5 张候选牌**；剩余牌源不足 5 张时，展示全部可用牌。明确增加格子或改变牌源的效果按其文本处理。

“剩余商店牌源”包括仍可供后续购买的未展示牌和当前未购候选。**展示不是购买，未购候选没有被消耗。** 成功购买的那一张退出普通商店牌源，之后不能在同一个牌源里再次被抽成候选，除非明确效果把它送回或新增另一份牌。

### 5.2.1 候选生成与拷贝

普通候选按随机结果从可用牌源选择；同一张具体卡牌不能在一批候选中重复出现。同名牌的两份拷贝是不同对象，可以同时出现。随机选取的精确概率分布仍需确认，不能把“随机”擅自解释为任意五张组合等概率。
...
本章将“仍可供购买”与“当前陈列在哪个区域”分开：未购候选始终没有被永久消费，但是否在陈列时就从额外卡组实体转入商店、额外卡组显示数量是否扣除这些候选，列为 **ECO-R01** 审阅项。它也影响刷新时的区域变化与 modifier，不能只用“放回”一词把这项未决问题掩盖。
...
3. 取消本次旧陈列，将未购候选重新纳入可选范围，与其他未展示牌一起生成新候选。
4. 成功完成后，消耗一次刷新次数。之前已经成功购买的牌不会因此被退回商店，星能支付也不撤回。
5. 完成刷新相关效果后，继续本人的购买窗口。

新候选可能再次包含刚才未购买的牌；刷新不保证出现不同牌。刷新不能恢复已经消费掉的额外卡组拷贝。
...
多项加减、设定价格、倍数、半价或目的地例外同时存在时，须有统一计算顺序及取整规则；这些非交换情况尚待审阅。不按卡面排列、来源稀有度或客户端展示顺序自行判定。
...
5. 将该牌移入最终目的地，默认是购买者弃牌堆；该拷贝退出普通商店牌源，同时消耗用于本次成功购买的一次性修正。
...
**购买的专门时序草案：** 本流程将支付／声明的后续延至购买成功或取消后，先完成明确的购买前反应；它是对[7.5 通用嵌套顺序](effects.md#多个效果与连续结算)的专门安排，需随本章审阅，不能暗中推广到放置或主动能力成本。
...
支付后若候选被另一效果移走，或者最终目的地变得无法接受该牌，如何处理购买完成、候选归属及退款，仍需具体裁定。这不同于支付前拒绝，也不能直接用程序报错作为游戏规则。
...
flowchart LR
  ...
  hand -->|合法放置| field["战场"]
  hand -->|弃置| discard
  field -->|随从死亡或法术清理| discard
```

图 5-B 是普通卡组成长与使用循环，具体区域继承依[6.5](lifecycle.md#区域转移与状态保留)。商店 → 弃牌堆 → 牌堆 → 手牌 → 战场为正向，转移不清除 modifier；手牌弃置、战场回弃牌堆则为逆向，清除 modifier。
