...
普通候选按随机结果从可用牌源选择；同一张具体卡牌不能在一批候选中重复出现。同名的两张牌是不同对象，可以同时出现。随机选取的精确概率分布仍需确认，不能把“随机”擅自解释为任意五张组合等概率。
...
**区域划分（已确认）：** 额外卡组只存放未陈列的牌，商店只存放已经陈列、当前可以购买的牌；一张牌同一时刻只在其中一个区域。上货是额外卡组 → 商店的真实转区，购买通常是商店 → 弃牌堆。

刷新先将商店中的全部未购牌退回额外卡组，洗牌后再从中取出最多 5 张进入商店；剩余不足时全部陈列，格子数量的明确修正仍适用。未购牌没有被消耗，但陈列时已经离开额外卡组，因此额外卡组数量只计算未陈列部分；“剩余商店牌源”则计算两个区域的合计。

**刷新退回额外卡组时保留 modifier（已确认）。** 例如，商店某张牌已有 +2 攻击修正，刷新退回后仍保有该修正，不恢复原始数值。此项专门继承规则见[6.5](lifecycle.md#区域转移与状态保留)，两个区域的定义见[2.6.3](elements.md#额外卡组与商店的区域划分)。
...
3. 将商店中的全部未购牌退回额外卡组，保留这些牌的 modifier；洗牌后，按当前商店格数取牌进入商店，默认最多 5 张、不足则全部取出。
...
本节沿用[图 2-B：区域迁移](elements.md#区域迁移示意)，不重复维护一张同样的牌组循环图。商店 → 弃牌堆 → 牌堆 → 手牌 → 战场为正向，转移不清除 modifier；手牌弃置、战场回弃牌堆则为逆向，清除 modifier。具体裁定见[6.5](lifecycle.md#区域转移与状态保留)。
