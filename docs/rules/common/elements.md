...
| 英雄卡 | 通过模式的英雄选择或英雄配置进入对局，与普通初始卡组／额外卡组的选牌途径区分；英雄选择与限制见第九章。英雄在基础类型上仍是随从牌。 |
...
图 2-B 展示常见区域迁移。实线必须由标注的行动或效果引发；虚线仅表示商店候选的来源，不表示卡牌实体自动迁移。连线本身不授予转移权限。
...
死亡、法术清理和湮灭虽然都可以将卡牌从战场送入弃牌堆，但不是同一种事件；各自的触发条件与后续效果分别判断。商店购买入弃牌堆沿用[旧规则书](../maintenance/sources.md#参考材料)的默认去向，报价、支付及候选更新等完整过程待第五章审阅。
...
modifier 的定义、完整裁定及例子统一见[6.5 区域转移与状态保留](lifecycle.md#区域转移与状态保留)。额外卡组尚未在这个序列中指定位置，不把它擅自插入商店之前；资源拾取后的消失不参与区域排序。
