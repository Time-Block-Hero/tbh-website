...
- **共存：** 普通放置后的法术占据战场格子，默认不能与另一场上法术或随从共占一格；资源牌的共存例外仍适用。玩家将法术放置到资源所在格，也会触发资源拾取；时停同格冲突按[湮灭](rounds.md#时停结束与湮灭)处理。
...
- **收益：** 资源牌文本带有[拾取](../reference/keywords.md#拾取)效果，为进入资源格的卡牌的控制者结算对应收益，例如获得 1 星能。收益由具体卡牌规定，不把该例子变成所有资源统一提供的数量。
...
“回合结束不消失”针对战场上的资源牌，不免除资源牌在手牌中通常适用的弃牌规则。拾取资格及收益归属由进入资源格的卡牌是否具有玩家控制者决定，统一见[6.8 拾取](lifecycle.md#拾取)。
...
图 2-C 只表示用于裁定 modifier 保留的顺序，**不是合法迁移路线，也不要求经过相邻区域**。向序列后方转移是正向，保留 modifier；向前方转移是逆向，清除 modifier，恢复原始状态。例如弃牌堆直接到战场属于正向；战场进入弃牌堆、返回手牌，或手牌／战场洗回牌堆，均属于逆向。

modifier 的定义、完整裁定及例子统一见[6.5 区域转移与状态保留](lifecycle.md#区域转移与状态保留)。额外卡组尚未在这个序列中指定一般位置；刷新退回保留 modifier 已有明确规则，见 2.6.3；资源拾取后的消失不参与区域排序。

### 2.6.3 额外卡组与商店的区域划分

**已确认：** 两者是互不重叠的实际区域。额外卡组存放尚未陈列的牌，商店存放当前已陈列的可购牌；同一张牌不会同时位于两处。上货把牌从额外卡组送入商店，购买把该牌从商店送到购买目的地。

刷新则先将未购牌全部从商店退回额外卡组，洗牌，再取最多 5 张进入商店。商店显示数量和额外卡组剩余数量各自计算；未购牌退回不是新生成，也不是退还先前的购买。

**刷新退回额外卡组保留 modifier**，不将这一步套用为清除修正的逆向转移。一般区域继承与此项明确例外见[6.5](lifecycle.md#区域转移与状态保留)；完整商店流程见[5.2.2](economy.md#陈列归属与区域)。
...
时停冻结战场与对手可见状态，包括星能和手牌数量；玩家可见本方本轮放置、入场及相关变化。双方可能独立放置到同一格，在时停末先处理湮灭等结束结算，再统一揭示结果。冻结不公开原本隐藏的手牌身份；完整规则见[3.3 时停](rounds.md#时停)。
