...
| 基础类型 | 属性与行为 |
| --- | --- |
| 随从牌 | 具有原始攻击、移动能力和生命等属性；进入战场后成为单位，维护当前生命、本回合剩余移动点及攻击次数等对局状态，依规则移动、攻击或发动能力。 |
| 法术牌 | 通过效果影响对局；普通放置后成为场上法术并占格，回合结束时移入弃牌堆。它没有随从的攻击、生命和速度属性。 |
| 资源牌 | 具有拾取效果，可与其他卡牌重叠，由玩家通过放置或移动卡牌触发拾取收益；拾取完毕后移出游戏，未被拾取时跨回合留场。详见下方规则。 |

英雄、野怪等描述身份，而非与上述三类并列的基础类型。英雄是具有英雄身份的随从，其开局位置、构筑资格和与胜负的关系由模式规定。野怪的生成、控制、敌我关系及奖励也须分别规定。其他地图物件同样以对应卡牌表示，不能仅凭“物件”名称推断它一定是资源牌或具有随从能力。
...
资源与资源重叠、将资源直接放到已有卡牌所在格是否立即触发拾取等边界，见[资源牌剩余边界](../maintenance/pending.md#资源牌剩余边界)。
...
| 攻击 | 战斗伤害引用的数值 | 随从属性；法术没有攻击属性，不等同于攻击为 0 的随从。 |
| 生命与最大生命 | 剩余生命与可恢复的生命上限 | 随从属性；法术未带生命属性，不因此被视为生命为 0 而死亡。 |
