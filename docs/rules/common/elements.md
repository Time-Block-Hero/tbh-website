...
**射线瞄准属于目标选择。** 先确定射线遇到的第一个随从，再检查它是否符合卡文的目标条件。若不符合，该方向是无效选择，不能跳过它寻找后面的合法目标。没有遇到随从时，也不能为要求随从目标的效果完成选择。选择窗口与无合法目标的处理引用[8.2](effects.md#目标模式与数量选择)；入场效果无法选择目标不撤销已经完成的放置。

例如「普通的子弹」可以选择任意关系的首个随从；「日炎枪手」要求这个首个随从是敌方。友方或无控制者野怪挡在敌方前面时，该方向对日炎枪手无效，而不是穿过它们射击后方敌人。射线选择不改变普通移动或攻击的格距算法。
