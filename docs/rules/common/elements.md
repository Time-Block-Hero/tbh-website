...
本章说明对象、属性、空间与区域。已确认条款作为裁定依据；尚缺参数的地方保留草案说明，集中见[第二章审阅事项](../maintenance/pending.md#第二章审阅事项)。
...
箭头可以指向周围八个方向。因此，箭头指向范围与普通移动的一步范围不同。箭头随卡牌朝向指向具体棋盘格；以该牌所属一方的朝向为基准，双方相对摆放时，箭头相对棋盘旋转半周。控制权改变是否改变朝向，留给控制权规则裁定。
...
**箭头层数（已确认）。** 一个有效方向有 X 层箭头，就向该方向对应格提供 X 个箭头计数，用于满足放置的箭头需求及计算建造的未固定值。层数不会延长箭头距离，也不产生新的方向；不同来源指向同一格的有效计数相加。放置与建造的条件及流程见[4.2](actions.md#放置与箭头)。其他效果写“每个箭头”时按方向数还是层数计数，仍见[箭头计数的剩余边界](../maintenance/pending.md#第二章审阅事项)。
