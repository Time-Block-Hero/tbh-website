...
本章说明行动、效果和系统流程共用的结算时序。已确认条款作为裁定依据；尚缺参数的地方保留草案说明，集中见[第八章审阅事项](../maintenance/pending.md#第八章审阅事项)。
...
| 行为 | 主定义与承诺边界 | 本章衔接 |
| --- | --- | --- |
| 放置 | [4.2.1](actions.md#放置步骤)：选择检查 → 明示成本 → 入场。 | 成本整体支付；入场先于拾取，拾取资格中途改变的边界另议。 |
| 移动 | [4.3](actions.md#移动与特殊位移)：普通与冲撞逐格，瞬移只处理终点。 | 逐格完成进入效果再走下一步；效果强制位移的支付口径仍待裁定。 |
...
| 主动能力 | [4.5.1](actions.md#发动步骤)：准备选择、支付并消费次数、反制和正文执行。 | 已付成本和次数不因反制返还；普通成本触发待主体完成，来源变化的剩余交互另议。 |
...
**窗口外的可选效果。** 文本允许执行或放弃时，先以相同概率选择执行／放弃；选择执行后，再从合法选项随机选取。目标数量不会改变放弃概率。强制效果没有放弃选项，多目标仍须满足原文数量条件；“至多 N”各数量之间的具体取样如会影响结果，须另明示，不能从二选一规则推导出未经确认的权重。
...
**SET-005 一份效果正文先完成（已确认）。** 执行期间每次变化仍完成必要的属性更新、死亡锁定与本体清理，明确的同时批次仍整体完成；因此产生的普通触发先记录，等这份效果正文完成后再处理。明示的攻击前／发动前反制，仍在自己的发生前节点处理。[8.2.4](#随机选择与随机伤害)的逐点随机伤害是明确的逐点结算机制，每点后完成后续再进入下一点。
...
同一来源进入时点完全相同、没有可区分的卡面顺序时的最后平局，仍须由生成该批来源的规则明确，不能临时采用内部编号作为玩法。

**同时变化与后续排序分开。** 例如一项效果同时对三个随从造成伤害，先整体完成伤害与死亡清理，之后才为产生的受伤／死亡效果排序；不能为了排序把同批伤害拆成逐个完整结算。多数情形已能由玩家与来源入场顺序区分，最后平局只用于这些条件仍完全相同的少见情形。创建顺序／棋盘扫描顺序目前仍是提案，尚未确认。
...
拾取者或资源在执行前变化的处理，统一跟进[资源牌剩余边界](../maintenance/pending.md#资源牌剩余边界)。
...
延迟事项到达指定时点后，纳入该时点的待结算事项。指定区域目标的离区与重入按[8.2.3](#无合法选项与目标失效)处理；数值读取采用[8.4.1](#数值运算与读值)默认，卡文明示提前记录则用该记录；同点普通效果使用[8.5.2](#同时满足条件的效果)的排序。时停中产生的慢速按[3.3.1](rounds.md#时停结束与湮灭)，在湮灭与拾取结算后、任何玩家时动前处理；其他阶段产生的慢速仍须明确时点。
...
事件数值记录和执行时读值采用[8.4.1](#数值运算与读值)；效果控制者的锁定、来源被封锁或控制权改变后的具体交互，继续跟进 **T-02**。未来时点尚未到来的安排不自动视为已经触发，见[8.6](#持续时间与失效)。
