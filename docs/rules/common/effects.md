...
| 主动能力 | [4.5.1](actions.md#发动步骤)：准备选择、支付并消费次数、反制和正文执行。 | 已付成本和使用次数不因反制返还；成本触发插入和来源变化的剩余交互仍待定。 |
...
伤害的专门顺序已确认：满足死亡条件时立即锁定死亡快照，先清理本体，再结算受伤及死亡等后续效果，见[6.3.1](lifecycle.md#确认死亡离场与死亡效果)。记录了受伤触发不表示它能抢在死亡锁定前执行。
...
图 8-B 表示需要定义的结算边界，不替未决排序作决定。明确的同时伤害先整体处理、锁定快照并清理全部本体，再处理触发，见[6.1.2](lifecycle.md#伤害事实与多段伤害)。新效果插在已有等待项之前还是之后、当前效果何时暂停、多个触发怎样排序，仍列为 **T-01**。不能从本图推导先进先出、后进先出或立即递归中的任何一种通则。

例如“造成伤害，然后抽牌”：按[8.7.1](#不同失败阶段)，伤害步骤成功才有后续抽牌；伤害达到死亡条件时先锁定死亡并清理本体，受伤后续不能撤销这次死亡；受伤效果与死亡效果怎样排序、何时回到原效果的抽牌步骤，仍须确定对应结算点。不能用文字中的“然后”跳过必要检查，也不能仅因记录了触发就强行暂停尚未完成的同时伤害批次。
...
延迟事项到达指定时点后，纳入该时点的待结算事项。指定区域目标的离区与重入按[8.2.3](#无合法选项与目标失效)处理；何时读取数值及与同一时点其他效果的排序，仍须明确。时停中产生的慢速按[3.3.1](rounds.md#时停结束与湮灭)，在湮灭后、任何玩家时动前处理；其他阶段产生的慢速仍须明确时点。
...
这里的“并行”确认两项效果没有成功依赖，不自行把它们规定为同一伤害批次；独立效果的结果先后和触发排序仍按对应时序裁定。**伤害被圣盾抵挡仍算成功执行（已确认）。** “造成 3 点伤害，然后抽一张牌”对合法目标执行伤害，伤害被圣盾全部抵挡，伤害事件已经发生，伤害步骤算成功，因此仍抽牌；供伤害量统计读取的数值则是 0。这不同于目标失效或伤害步骤被取消而根本没有执行。执行成功与实际造成多少伤害分别判断，不能由计数为 0 自动推断执行失败。

例如已合法选择两名目标，一名在结算前离开原区域，仍对另一名执行；选取时必须选两名，不代表执行时必须仍有两名。本条不授予重新选择权。多个目标部分成功能否满足后续“然后”的成功门槛、治疗无实际恢复是否算执行成功，仍须单独明确。
...
触发资格与事件记录、控制者和数值分别在何时锁定，来源被封锁或控制权改变后的具体交互，继续跟进 **T-02**。未来时点尚未到来的安排不自动视为已经触发，见[8.6](#持续时间与失效)。
...
死亡确认后的内部顺序已按[6.3](lifecycle.md#死亡与消灭)定为先清理本体、再结算死亡效果。满足死亡条件时按快照锁定并先清理本体，后续受伤效果不能通过回复撤销死亡；胜负检查的插入时点仍未确认；也不能因为流程图还画有下一步，在模式已经判定对局结束后强行继续。
