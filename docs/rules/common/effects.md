...
| --- | --- | --- |
| 放置 | [4.2.1](actions.md#放置步骤)：选择检查 → 明示成本 → 入场。 | 成本引发的变化、持续效果、拾取和入场效果的先后。 |
...
**入场时序提案（待确认）：** 卡牌成功进入战场后，轮到某项入场效果处理时再为它选择目标。入场选择不属于放置承诺前的取消窗口；不能在看到抽牌、拾取或其他入场结果后，把已经完成的放置撤回。是否采用此方案，以及与拾取的先后，见 8.5.3。
...
例如“造成伤害，然后抽牌”：伤害完成后会记录受伤触发，但该触发在死亡检查之前还是之后处理，以及何时回到抽牌，必须由伤害与效果的结算点共同决定。不能用文字中的“然后”跳过必要检查，也不能仅因记录了触发就强行暂停尚未完成的同时伤害批次。
...
**SET-006 到期与结算分开判断。** 可设计的时间维度见[7.5](card-effects.md#执行时间存在期限与限次)；本节处理到达时点或条件变化后的局面。

持续效果失去生效条件时停止提供对应贡献，依[6.2](lifecycle.md#属性变化)重新计算。一次性赋予的修正按自身期限处理；逆向转区清除 modifier 则依[6.5](lifecycle.md#区域转移与状态保留)。这些是独立原因，不把一切变化都等到回合末统一清空。
