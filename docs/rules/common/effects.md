...
**同批入场顺序（已确认）。** 同一批进入战场的来源，在上述玩家顺序内按入场格的棋盘固定顺序“从上到下、从左到右”区分，不随玩家视角翻转。这只确定来源及后续效果先后，不将本批入场拆成逐张完整结算。同格及其他区域仍完全同序的最后兜底，另按[T-01](../maintenance/pending.md#第八章审阅事项)跟进，不擅自采用内部编号。
