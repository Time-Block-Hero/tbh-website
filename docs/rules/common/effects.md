...
**SET-012 战场目标离场后失效（已确认）。** 一项效果指定的战场目标离开战场后，就不再是这次效果的有效目标。即使同一张牌在效果结算前再次进入战场，也不会恢复这次目标关系；效果明确允许跨区域追踪时，按其例外处理。
...
例如“造成伤害，然后抽牌”：伤害达到死亡条件时先锁定死亡并清理本体，受伤后续不能撤销这次死亡；受伤效果与死亡效果怎样排序、何时回到原效果的抽牌步骤，仍须确定对应结算点。不能用文字中的“然后”跳过必要检查，也不能仅因记录了触发就强行暂停尚未完成的同时伤害批次。
...
延迟事项到达指定时点后，纳入该时点的待结算事项。指定战场目标的离场与重入按[8.2.3](#无合法选项与目标失效)处理；何时读取数值及与同一时点其他效果的排序，仍须明确。“慢速”的准确结算时点尚未裁定，不把它自动解释为下回合开始。
