...
**随机选择在正文执行时进行（已确认）。** 卡文明示的随机选取、随机分配不在慢速入场准备时预抽或锁定候选名单，而在时停末轮到该效果执行时，从当时的合法候选中选取。因此，本回合对手新放置且在合并、湮灭、拾取及此前效果后仍合法的随从，也可成为候选；不是在时停刚结束时为所有待结算事项统一抽样。已由玩家准备并锁定的目标，不因窗口关闭而改成重新随机。快速效果的随机选择随其正文当场执行。
