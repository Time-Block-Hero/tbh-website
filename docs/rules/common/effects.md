...
**入场目标选择：选择快速，结算按效果速度。** 不以入场目标条件代替普通放置条件。独立时停中，默认慢速入场在卡牌成功放置、入场效果触发后，立即在本人的窗口完成可以确定的目标、模式、方向、数量与点数分配，再登记正文到时停结束；准备选择不提前执行正文；结算时重验指定关系，不因窗口已关闭而重新随机选择目标。依赖正文中新信息的选择在执行到对应步骤时处理，窗口外采用合法随机。快速入场的选择和正文都在当次处理完成；时停外的入场也在当次结算中选择并执行。完整放置流程见[4.2.1](actions.md#放置步骤)。可建造卡牌自身入场仍在完成建造时才触发，见[4.2.3](actions.md#建造)。
