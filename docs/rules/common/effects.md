...
| 行为 | 主定义与承诺边界 | 本章衔接 |
| --- | --- | --- |
| 放置 | [4.2.1](actions.md#放置步骤)：选择检查 → 明示成本 → 入场。 | 成本整体支付；入场触发／准备先于拾取，正文按快速或默认慢速时点处理；拾取开始时的资格重检见[6.8.1](lifecycle.md#拾取完成流程)。 |
...
**普通触发的准备选择：选择快速，结算按效果速度。** 独立时停中，入场、移除、被保留等普通卡牌触发默认慢速。触发条件满足后，立即在效果所属玩家的窗口完成可以确定的玩家目标、模式、方向、数量与点数分配，再登记正文到时停结束；准备选择不提前执行正文，具体速度与例外见[3.3.2](rounds.md#待结算效果的登记与顺序)。结算时重验指定关系，不因窗口已关闭而重新随机选择目标。依赖正文中新信息的选择在执行到对应步骤时处理，窗口外采用合法随机。快速效果的选择和正文都在当次处理完成；时停外的普通触发也在当次结算中选择并执行。

入场效果在卡牌成功放置、入场触发后开始准备，不能以其目标条件代替普通放置条件；完整放置流程见[4.2.1](actions.md#放置步骤)。可建造卡牌自身入场仍在完成建造时才触发，见[4.2.3](actions.md#建造)。

直线射击准备仅固定发射原点与方向，首个命中随从在正文执行时确定，依[2.5.1射线范式](elements.md#直线效果范围)。它不是对某个随从的提前锁定，不适用“原目标失效后禁止替换”来阻止沿线新目标被命中。

**随机选择默认在正文执行时进行（已确认）。** 除已明确指定提前随机的效果外，卡文明示的随机选取、随机分配不在慢速效果准备时预抽或锁定候选名单，而在时停末轮到该效果执行时，从当时的合法候选中选取。因此，本回合对手新放置且在合并、湮灭、拾取及此前效果后仍合法的随从，也可成为候选；不是在时停刚结束时为所有待结算事项统一抽样。已由玩家准备并锁定的目标，不因窗口关闭而改成重新随机。快速效果的随机选择随其正文当场执行。

**矿石扫描的提前随机特例（已确认）。** 入场触发后，立即从己方领土的完全空格中等概率、不重复地随机选取2格；不足2格则全部选取，没有空格则选空集合。系统自动完成这一步，不让玩家选择0／1／2，也不提前生成资源。独立时停中，默认慢速正文在时停末分别于这些已确定格子生成1张「遗失的智慧」（FNG-UN-003）。执行时再次检查[资源生成要求的完全空格](elements.md#棋盘与空间关系)；某格已被任何卡牌占用，则跳过该格，不补选其他格、不重新随机。时停外入场仍按通则在当次结算中完成选择和正文。该卡的“最多2”仅处理候选不足，不提供玩家主动少选或放弃选项；此特例不改变其他慢速随机效果的默认时点。
...
**放弃须由文本允许（已确认）。** 矿石扫描“最多2”的候选不足特例依[8.2.1](#选择时点与操作窗口)，不产生数量菜单。其他效果默认强制执行；“可以”“至多”等文本才允许放弃或少选。窗口外自动选择也须遵守相同选项范围。窗口外的可选放弃与随机权重按[8.2.4](#随机选择与随机伤害)处理，不默认最大收益、最多目标或总是放弃。

**分配是目标选择的一种（已确认）。** 分配伤害、护甲及其他点数共用本规则：在合法候选中逐点选择，同一目标可以重复选中，直到分配总点数等于效果规定的总量。每个实际接受者至少分得1点整数，可以全部分给同一合法目标；没有合法目标时该部分不执行。先完成全部选择并汇总各目标所得点数，再在该效果的结算时点同批赋予，不在分配每一点时提前施加伤害、护甲或插入后续效果。随机分配同样先选完再结算，取样方式见[8.2.4](#随机选择与随机伤害)。已锁定分配中的目标失效时，按[8.2.3](#无合法选项与目标失效)跳过其份额，不重新分给其他目标。
...
**随机分配逐点取样，同批结算（已确认）。** 无论卡文明示随机，还是因没有玩家操作窗口而由系统代选，分配总计 N 点时，均从本次合法候选中逐点等概率、有放回地抽取目标；同一目标可以重复选中。先完成 N 次取样，按目标汇总点数，再同批结算。选择点数期间不造成伤害或赋予护甲，不因某个目标将承受致死伤害而提前把它从候选中移除；死亡及普通后续依[8.5.4](#批量结算通则)处理。没有合法候选时该部分不执行。例如向 A、B 分配3点，抽样得到 A、A、B，则同批对 A 结算2点、对 B 结算1点；不是三次独立伤害。只有卡文明确要求逐次造成伤害，才是分别结算的多次效果。

**窗口外的可选效果。** 本段不适用于矿石扫描已明定的自动选满特例。其他文本允许执行或放弃时，先以相同概率选择执行／放弃；选择执行后，再从合法选项随机选取。目标数量不会改变放弃概率。强制效果没有放弃选项，多目标仍须满足原文数量条件；“至多 N”先等概率决定放弃／执行，执行时在1至实际可选上限之间等概率选择数量，再合法随机选择目标。
...
**SET-005 一份效果正文先完成（已确认）。** 执行期间每次变化仍完成必要的属性更新、死亡锁定与本体清理，同类批量变化依[8.5.4](#批量结算通则)整体完成；因此产生的普通触发先记录，等这份效果正文完成后再处理。明示的攻击前／发动前反制，仍在自己的发生前节点处理。[分配](#多目标与点数分配)逐点完成选择后同批结算，不构成逐点插入后续的例外。
...
图 8-B 说明子效果先于等待中的其他效果；不允许普通触发打断尚未完成的一份效果正文或同一同时批次。来源离场通常不取消已触发效果；湮灭取消对应入场的待结算慢速入场是明确例外，见[湮灭](../reference/keywords.md#湮灭)。目标合法性仍逐项检查。
...
同一事件使多个效果符合条件，不表示它们在同一局面上完成；先结算者可以改变后一个的目标或可执行内容。玩家不能自由重排这一组效果。同步时停仍分别处理各自冻结局面；已经登记的时停末队列专门按[3.3.2 确认提交顺序](rounds.md#待结算效果的登记与顺序)执行，不以本条的玩家顺序重新排列。跨双方部署合并依第三章处理。
...
相对顺序以[4.2.2](actions.md#放置与拾取)为唯一主定义；默认慢速入场的目标准备及登记依[8.2.1](#选择时点与操作窗口)，时停末队列依第三章的确认提交顺序，其他同时触发依本章排序。建造中卡牌自身入场仍在完成建造时才触发，主定义见[4.2.3](actions.md#建造)，不是放置时先触发后等待解锁。
...
**通则的边界。** “召唤A，然后召唤B”有明确先后及成功依赖，不自动合并成同批；[随机分配](#随机选择与随机伤害)的逐点取样不拆开结算批次。数份独立效果同一时点触发，也不因此合并成一份批量正文。通则决定结算边界，不授予额外目标、占格许可或不同的费用规则。
...
**SET-011 已建立的延迟事项不依赖来源继续存活（已确认）。** 一项效果已经完成安排、建立了未来执行的事项后，不因来源随后死亡或离场而自动取消；文本明确要求“若来源届时仍在场”等条件时，按该条件检查；[湮灭取消慢速入场](../reference/keywords.md#湮灭)另按专门规则处理。执行时仍检查目标是否合法，不因安排保留就保证操作成功。

「刺客-J」的当前卡文为“入场：选定一个非英雄单位，将其消灭。”入场效果选定目标并建立延迟消灭后，即使刺客-J 在慢速结算前死亡，该事项仍保留；到期时对仍合法的目标执行消灭。卡文不增加“刺客-J 仍在场”的条件。
...
**SET-010 已触发的效果通常不因来源离场而取消（已确认）。** 效果已经触发、正在等待结算时，来源随后死亡或离场，不自动取消这项效果。尚未触发的效果则仍须在触发条件到来时满足自身的存在与生效条件。

**湮灭例外：** 对随从与法术共同适用，取消其本次入场已登记但尚未结算的慢速入场效果；消灭仍允许这些效果完成。不是撤销整段历史，详见[12.10](../reference/keywords.md#湮灭)。
