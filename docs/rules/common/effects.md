...
**入场目标选择：选择快速，结算按效果速度。** 不以入场目标条件代替普通放置条件。独立时停中，默认慢速入场在卡牌成功放置、入场效果触发后，立即在本人的窗口完成可以确定的玩家目标、模式、方向、数量与点数分配，再登记正文到时停结束；准备选择不提前执行正文；结算时重验指定关系，不因窗口已关闭而重新随机选择目标。依赖正文中新信息的选择在执行到对应步骤时处理，窗口外采用合法随机。快速入场的选择和正文都在当次处理完成；时停外的入场也在当次结算中选择并执行。完整放置流程见[4.2.1](actions.md#放置步骤)。可建造卡牌自身入场仍在完成建造时才触发，见[4.2.3](actions.md#建造)。

直线射击准备仅固定发射原点与方向，首个命中随从在正文执行时确定，依[2.5.1射线范式](elements.md#直线效果范围)。它不是对某个随从的提前锁定，不适用“原目标失效后禁止替换”来阻止沿线新目标被命中。

**随机选择默认在正文执行时进行（已确认）。** 除已明确指定提前随机的效果外，卡文明示的随机选取、随机分配不在慢速入场准备时预抽或锁定候选名单，而在时停末轮到该效果执行时，从当时的合法候选中选取。因此，本回合对手新放置且在合并、湮灭、拾取及此前效果后仍合法的随从，也可成为候选；不是在时停刚结束时为所有待结算事项统一抽样。已由玩家准备并锁定的目标，不因窗口关闭而改成重新随机。快速效果的随机选择随其正文当场执行。
