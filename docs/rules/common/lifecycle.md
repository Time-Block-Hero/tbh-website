...
图 6-E 只规定同批伤害、死亡清理和后续效果三者的先后，不规定多个触发之间的排序。光环失效等引发的连带死亡怎样插入、多个伤害是否构成同一批，仍须在对应时序明确。

**LIF-016 伤害计数不包含过量部分（已确认）。** 用于“造成／受到多少伤害”及伤害进度累计的数值，等于经过改写、防止、圣盾和护甲之后实际用于扣减生命的伤害量，但最多计至受击前剩余的生命量。被防护抵挡的部分与超过剩余生命的部分不计入该值。

例如目标受击前剩余 2 生命，穿透防护的伤害为 5，本次伤害计数为 2；1 生命随从承受 10 点未抵挡伤害，伤害进度只增加 1。攻击力或伤害数值再高，也不能靠过量部分额外积累进度。本条规定事件读值，不同时裁定当前生命是否允许记录负数，也不增加“无限攻击力”这一属性取值。
...
**已确认：生命 buff／debuff 同时等量影响生命上限与当前生命。** 获得 +N 生命时，两者都增加 N；受到 −N 生命修正时，两者都减少 N。修正到期或被撤去时，撤销相同的数值变化，而不是只把当前生命限制在新上限内。

例如当前 2／上限 5，获得 +3 生命后为 5／8；没有其他变化时，失去这份加成回到 2／5。当前 2／上限 5，受到 −2 生命修正后为 0／3，立即按[6.3](#死亡与消灭)锁定死亡并处理。

属性修正导致的当前生命变化不等同于治疗或伤害，不自动产生对应的受伤／治疗事件。等量扣除不在 0 截断，例如剩余 1 生命受到 −3 生命修正后为 −2，并立即按死亡快照处理；变身切换基础属性时与当前值怎样联动另按变身规则；本条不把所有设定上限或变身效果等同 buff／debuff。

### 6.2.2 速度与剩余行动点

速度表示当前移动额度的计算依据，剩余行动点表示本回合尚可花费的额度。移动花费行动点不会降低速度。

**已确认：速度 buff／debuff 同时等量影响当前速度与剩余行动点。** 获得 +N 速度，两者都增加 N；受到 −N 速度修正，两者都减少 N；撤销修正时同样反向调整。不能只降低速度上限而保留原行动余额。

例如当前速度 5、剩余行动点 3，受到 −2 速度修正后成为速度 3、剩余行动点 1。回合开始的刷新仍依[3.2](rounds.md#回合开始)，一次加速不自动补满整回合额度。等量扣除不在 0 截断：剩余行动点 1 时受到 −3 速度修正，剩余行动点为 −2，不能支付移动成本；撤销该修正后回到 1。
...
- **一次性进度只自然完成一次。** 完成时立即标记已完成，停止后续累计；不等奖励结算完毕才标记。普通一次性进度可按下文逆向转区规则重置；其他重新开启或外部直接触发已完成任务的许可须明示。
...
「迷离之剑-烬白」的拾取变身进度为**一次性进度**；其他本轮整理的机械天国累计效果采用循环进度。变身发生时保留当前状态，见[6.6.2](#升级变形与复制)。**普通一次性进度的完成标记也属于逆向重置型状态（已确认）**：逆向转区时连同当前进度重置为未完成，可重新累计；明确跨区保留的特殊任务例外。烬白变身后逆向还原，再次入场能够重新完成进度并变身。不同形态如何对应各项进度记录、哪些区域允许计数，以及外部直接触发已完成任务的许可，继续跟进 **LIF-R10**。建造仍由[4.2.3](actions.md#建造)规定，不自动采用这里的循环扣除机制。
...
**实际累计与“视作”分开。** 「烈阳圣女-阿蕾缇娅」在效果结算时额外视作保留两回合，「烈阳恩典」「高等阳炎术」也有视作保留的文本。其附加值如何影响读取，以及是否产生“每次被保留时”的触发，不能仅凭本条推导；统一跟进 **LIF-R11**。“本回合获得保留”先保护本次弃牌、更新计数，再到期，顺序见[3.6](rounds.md#回合结束)。唱诗班诗童等保留触发与其他结束效果的相互排序仍须细化。
...
快照锁定的是**这次已经成立的死亡**，同一次死亡只处理一次，不反复重判。后续效果使其他随从新满足死亡条件时，那是新的死亡事件，另行记录与处理；不能用旧快照排除后续真实发生的死亡。明确的同时伤害按[6.1.2](#伤害事实与多段伤害)先处理全部伤害、再锁定本批快照并清理全部本体；清理本体、光环失效等连带变化产生新死亡时的具体插入顺序，仍由 T-01 细化。
...
3. 再处理本次死亡引发的有效死亡效果，使用快照中的离场前信息。本体已经离场，不会仅因此丢失本次应结算的死亡效果。已经触发的受伤效果也不因来源离场而取消，见[8.7.2](effects.md#来源离场封锁与控制权变化)；它与死亡效果的相互排序仍由 T-01 裁定。

例如“死亡时，在其所在格子上召唤一个僵尸”，使用死亡随从离场前的格子。先清理死亡随从，便不会被它自己的本体阻挡召唤；若该格因其他变化已经被占用，仍检查召唤的实际合法性，不获得无视占格的许可。

多个死亡及其他后续效果如何排序、胜负检查插在哪里，以及除位置外还需保留哪些数值，继续见[审阅事项](../maintenance/pending.md#第六章审阅事项)。这些边界不改变上述已确认的本体与死亡效果先后。
...
耐久及明确跨卡组循环的历史记录按[6.5.1](#状态的转区保留类型)保留，不属于逆向转区一并重置的 modifier；[保留回合数](#被保留的回合数)则采用正向保留、逆向清零。当前生命、移动额度、建造进度等其他实例数据，在“恢复原始状态”时的逐项初始化，以及清除与离场／入场触发的先后，仍须在对应结算条款明确；不能仅凭本条把全部实例记录都视作 modifier，也不能因此另建卡牌身份或改变拥有者。
...
每一种状态的规则必须交代：记录对象、初始值、更新／消耗条件、有效区域、转区保留类型，以及到期或刷新条件。尚未规定的生命与移动额度初始化、建造记录和特殊任务例外不能仅按名称强行归类；具体缺口见 LIF-R08／LIF-R10／LIF-R12。复制、变形和更换控制者另按各自规则处理，不由跨区保留类型直接决定。
...
| 后天获得的圣盾 | 有／无两态；重复获得不叠加抵挡次数。 | 逆向重置型，清除获得的状态。 | 抵挡后消除；原生圣盾重入会恢复，主定义见[圣盾](../reference/keywords.md#圣盾)。 |
...
| 剩余行动点 | 当前可用于移动的额度。 | buff／debuff 等量联动见[6.2.2](#速度与剩余行动点)；入场初始化与建造完成另依对应规则。 | 不能用攻击／能力次数刷新推导具体移动额度。 |
