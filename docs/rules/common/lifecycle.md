...
建造进度的累计、固定、封锁及解锁统一见[4.2.3 建造](actions.md#建造)。本节只处理实例记录的生命周期：完成时是否恢复行动额度、是否补触发入场，以及转区后的进度初始化，见 LIF-R08。

**实例记录可以有不同的保留期限。** 剩余耐久和“一张牌在本局累计被打出几次”等跨卡组循环记录，不随逆向转区清除 modifier 而重置。统计期间为本回合的能力额度仍按自己的刷新规则处理；转区保留类型与状态／buff 速查统一见[6.5.1](#状态的转区保留类型)及[6.5.2](#特殊状态与-buff-速查)。主动能力默认见[4.5](actions.md#主动能力)，记录的设计维度见[7.6.8](effect-design/dictionary/usage.md)。
...
「迷离之剑-烬白」的拾取变身进度为**一次性进度**；其他本轮整理的机械天国累计效果采用循环进度。一次性完成标记怎样跨区继承、变形如何承接记录、哪些区域允许计数，以及外部直接触发已完成任务的许可，继续跟进 **LIF-R10**；当前进度归零不自动表示已完成任务重新开放。建造仍由[4.2.3](actions.md#建造)规定，不自动采用这里的循环扣除机制。
...
耐久及明确跨卡组循环的历史记录按[6.2.4](#建造与其他实例记录)保留，不属于逆向转区一并重置的 modifier；[保留回合数](#被保留的回合数)则采用正向保留、逆向清零。当前生命、行动额度、建造进度、能力使用记录等其他实例数据，在“恢复原始状态”时的逐项初始化，以及清除与离场／入场触发的先后，仍须在对应结算条款明确；不能仅凭本条把全部实例记录都视作 modifier，也不能因此另建卡牌身份或改变拥有者。
...
| 一次性进度完成标记 | 该项一次性进度是否已经自然完成。 | 跨区／变形后的继承待 LIF-R10 裁定，不能由当前值归零推断重新开放。 | 完成时立即标记，见[6.2.6](#进度)。 |
