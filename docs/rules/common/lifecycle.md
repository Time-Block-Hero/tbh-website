...
**令卡牌被保留一次（已确认）。** 执行一次真实的保留结算：先将该牌实际保留回合数加1，再按其有效效果触发“被保留时”的后续。例如「烈阳恩典」使手牌中的「沐阳晶灵」立即被额外保留一次，既增加计数，也触发它的永久卡面成长；不是只增加用于计算伤害的读值。此动作不推进游戏回合，不执行整轮弃牌或其他回合结束流程。同回合多次明确的额外保留分别执行，也不占用回合末正常保留的那一次。仅执行保留一次并不自动授予[保留](../reference/keywords.md#保留)词条；烈阳恩典另写“本回合获得保留”才提供本回合弃牌豁免。
