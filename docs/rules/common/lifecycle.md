...
5. 剩余伤害扣除目标当前生命。立即检查死亡条件，锁定死亡快照并先清理本体，再处理受伤及死亡等后续效果，按[6.3.1](#确认死亡离场与死亡效果)处理；同批与嵌套效果的细分边界见第八章。
...
**尚需裁定：** 目标剩余生命为 2 而穿透防护的伤害为 5 时，供“受到多少伤害”引用的量算 5 还是 2；本稿不把超额伤害自动截断。死亡快照先于受伤后续结算的规则见[6.3.1](#确认死亡离场与死亡效果)；同时伤害的批次边界和嵌套触发顺序仍见 LIF-R03／T-01。
...
快照锁定的是**这次已经成立的死亡**，同一次死亡只处理一次，不反复重判。后续效果使其他随从新满足死亡条件时，那是新的死亡事件，另行记录与处理；不能用旧快照排除后续真实发生的死亡。多目标同时伤害的批次边界，以及清理本体、光环失效等连带变化产生新死亡时的具体插入顺序，仍由 T-01 细化。
