...
本章说明伤害、状态变化与卡牌生命周期。已确认条款作为裁定依据；本轮规则已固定为试玩基线，集中见[第六章审阅事项](../maintenance/pending.md#第六章审阅事项)。
...
**圣盾先于护甲已确认。** 以下流程统一处理伤害数值、防护与死亡快照：
...
**LIF-015 同时伤害整体处理（已确认）。** 依[8.5.4批量结算通则](effects.md#批量结算通则)，一次对多张牌的伤害先完成全部伤害变化，再锁定本批死亡快照并清理所有死亡本体，之后才处理受伤、死亡等后续效果。不能完整结算第一个目标及其触发后，再对第二个目标造成这批伤害；也不能因一方先被计算为致死而取消同批已经确定的反击伤害。
...
图 6-E 规定同批伤害、死亡清理和后续效果的先后；普通触发排序引用[8.5](effects.md#多个效果与连续结算)。默认批次与明示逐次例外依[8.5.4](effects.md#批量结算通则)，不再分别为每种批量动作提出先后问题。
...
死亡快照与清理见[6.3.1](#确认死亡离场与死亡效果)；同批后续与嵌套顺序见[8.5](effects.md#多个效果与连续结算)。
...
直接消灭、减少生命上限或明确支付生命，与伤害分开判断；不能默认用圣盾或护甲支付这些变化。生命成本允许足额支付至死亡，支付与后续见[8.3.2](effects.md#成本本身改变局面)。

**食锈螯兽的生命吸取（已确认）。** 随机选择一个己方机械或建筑随从，给目标施加生命上限−3，给自身施加生命上限＋3；当前生命同步等量变化。这是属性修正，不是造成伤害或治疗，圣盾／护甲不抵挡，也不产生受伤／治疗事件。两项为同一批属性变化，完成后处理必要死亡与后续；没有合法目标时不获得自身加成。

## 6.2 属性变化

**LIF-004 原始值、修正与当前值分开。** 原始属性来自[卡牌信息](elements.md#通用属性与各类型专属属性)，对局中生效的 modifier 影响具体实例。原始定义没有因受伤、行动消耗或暂时加成而被改写。明确的永久卡面成长另按[6.2.8](#永久卡面成长)改变该实例的基础数值。

**全属性（已确认）**只指攻击、生命上限与速度这三个卡面属性；不包括费用、攻击范围、护甲、耐久、箭头或效果数值。生命和速度增加时，当前生命与剩余行动点按下述规则等量联动。
...
**恢复行动点（已确认）。** 恢复最多达到当前速度上限，不提高速度。速度3且剩余行动点3时，天庭乐师恢复1点后仍为3。明确“获得额外行动点”须另行说明超限许可，不能从“恢复”推导。
...
- **建造目标值**：对应卡牌规定的完成要求，用于判断建造进度是否已满；可建造牌的“箭头需求 N”提供此目标值，普通放置门槛另按[4.2.3](actions.md#建造)处理。

箭头如何增加建造值、何时固定以及满值后的形态解锁，统一按[4.2.3 建造](actions.md#建造)处理。初次建造及逆向转区后重新建造，固定值为 0，未固定值依当时箭头建立；具体完成与固定时点仍引用该主定义。

建造是卡牌当前形态的独立规则记录。沉默不跳过建造、不清除建造值；完成建造后仍保留沉默，因此被封锁的入场效果不触发。明确赋予固定建造值时直接增加已固定值并立即检查完成。
...
**填满进度（已确认）：** 效果要求“填满进度”时，将所选有效且未完成进度的当前值补至其当前有效目标值，再按上文的正常达标流程登记完成。循环进度扣除这次目标值后回到0；一次性进度标记已完成。已经完成的一次性进度不会因“填满”再次完成。目标值采用此时生效的值，包括临时改动，不是卡面的原始目标。填满只补足缺少的进度，不额外叠加一个目标值，也不绕过正常的完成登记与后续时序。

例如某循环进度当前为2/5，填满后补到5/5，登记一次完成并回到0/5；若其有效目标已降至4，则补到4/4后完成，回到0/4。奖励执行前已经扣除进度；奖励被反制也不返还。一次性进度从2/5填满后标记完成，之后再对它使用填满不会重复领取奖励。

“填满一张卡牌的进度”作用于该卡牌当前各项有效、尚未完成的进度；没有这样的进度时不作为填满候选。选择多张卡牌时按不同卡牌计数，同一张牌不因有多项进度占用多个名额。来源失效、建造中等导致进度效果当前无效时，不因填满而绕过其封锁。

**直接触发：** 效果明确要求“触发一个进度效果”时，令选定进度效果产生一次完成效果并进入连锁，不增加、扣除或重置原进度，也不将这一操作记作自然完成。即使一次性进度已完成，外部仍可令它执行一次完成效果；不重开任务，也不改变完成标记。完成效果被反制，不返还已经扣除的循环进度、不撤销已登记的一次性完成标记。对象的选择、合法性和操作窗口仍依原效果要求。“填满”采用上一段的达标流程，不属于这里的直接触发。
...
**机械天国的具体进度选择（已确认）。** 天庭乐师计入自己的完成效果。集群主脑在完成效果实际执行时，从己方在场且具有有效未完成进度的卡牌中随机选择三张不同的卡牌；不足三张则全部选择，再填满所选卡牌的进度。借时蜉蝣填满其正上方格子中合法卡牌的进度；若该格有多张合法卡牌，按操作窗口选择一张，窗口外由系统合法随机选择。两者均采用填满规则，不采用保留原计数的直接触发规则。

**建造目标降低的对局规则。** 泰拉理工学院完成进度后，建造目标减免作为该玩家本局持续规则叠加，作用于目前与未来归其控制的可建造随从，目标最低为0；不追溯取消已完成的建造。该对局规则已产生后不依赖学院继续在场。

### 6.2.7 被保留的回合数

**LIF-011 保留回合数（已确认核心）。** 「被保留的回合数」是卡牌实例的累计状态，初始为 0。每次回合结束，该牌实际被保留在手牌中时，计数增加 1；同一次回合末清理不重复计数。效果明确令该牌“被保留一次”时，也实际增加一次计数，详见下文；这一额外保留与回合末正常保留分别计算。仅仅获得[保留](../reference/keywords.md#保留)词条，或在回合中进入手牌，不会立即增加计数。
...
**令卡牌被保留一次（已确认）。** 执行一次真实的保留结算：先将该牌实际保留回合数加1，再按其有效效果触发“被保留时”的后续。例如「烈阳恩典」使手牌中的「沐阳晶灵」立即被额外保留一次，既增加计数，也触发它的永久卡面成长；不是只增加用于计算伤害的读值。“被保留时”的普通触发也采用[默认慢速](rounds.md#待结算效果的登记与顺序)：独立时停中，实际计数按这次保留结算先更新，未标快速的成长等正文等待时停末；时停以外按当次结算顺序处理。此动作不推进游戏回合，不执行整轮弃牌或其他回合结束流程。同回合多次明确的额外保留分别执行，也不占用回合末正常保留的那一次。仅执行保留一次并不自动授予[保留](../reference/keywords.md#保留)词条；烈阳恩典另写“本回合获得保留”才提供本回合弃牌豁免。

**实际累计与“视作”分开。** 「烈阳圣女-阿蕾缇娅」的“保留回合数视作＋2”只影响效果读值，不改变实际保留记录，也不额外产生“每次被保留时”的触发。例如实际累计2次、视作额外2次时读取4，实际记录仍为2，不额外触发沐阳晶灵。单纯设置或复制保留计数也不等于实际执行若干次保留，不能只因读值增加就补发保留后续。

“本回合获得保留”先保护回合末本次弃牌、更新实际计数，再到期，顺序见[3.6](rounds.md#回合结束)。回合末保留判断、清理批次及后续触发统一采用该节；结束效果产生的手牌会进入随后快照，清理后续产生的手牌不追加、不增加本次回合末保留计数。

### 6.2.8 永久卡面成长

**永久卡面成长（已确认）。** 效果明确直接提高某张牌的卡面数值时，改变的是这张卡牌实例的基础数值，不是给它附加一份 modifier，也不改变其他同名牌共享的原始定义。已经获得的成长跨区域保留，逆向转区不清除，沉默也不清除；重置及重新入场时仍以成长后的实例卡面数值作为属性依据。

「沐阳晶灵」每次实际被保留时，卡面攻击、生命、速度与攻击范围永久＋1；**费用不变，效果文本中的＋1也不增长**。初始费用3、四项数值均为1，第一次成长后费用仍为3、四项数值均为2。保留计数本身仍会逆向归零，不会带走已经获得的卡面成长。

沉默可以封锁尚未触发的成长效果，使其不能继续触发，但不撤回已经产生的基础数值改变。普通 buff、没有注明期限的 buff 与明确的永久卡面成长分开；不能只因效果持续较久，就认为它能抵抗沉默或逆向重置。永久变身的形态规则另见[6.6.2](#升级变形与复制)。
...
| 献祭自己的随从 | 否 | 按献祭效果或成本说明处理；不触发死亡效果。默认送入当前归属玩家弃牌堆，卡文明示其他去向时例外；不从“献祭”推断为消灭。 |
...
击杀奖励在死亡本体及耐久等必要处理完成后，作为死亡后续为直接致死来源所属玩家结算；无玩家归属的来源不领取玩家收益，不额外等到回合结束。模式只在明示不同奖励规则时覆盖。
...
各来源洗回牌堆的 modifier 处理见[6.5](#区域转移与状态保留)。随机取样见[8.2.4](effects.md#随机选择与随机伤害)；多张牌置于牌堆顶／底未指定次序时，按其原区域中的先后排列；原区域无顺序时按8.5.2来源顺序。
...
耐久及明确跨卡组循环的历史记录按[6.5.1](#状态的转区保留类型)保留，不属于逆向转区一并重置的 modifier；[保留回合数](#被保留的回合数)则采用正向保留、逆向清零。当前生命、剩余行动点和圣盾有无属于逆向重置的属性，处理见[6.5.2](#特殊状态与-buff-速查)；建造记录依[4.2.3](actions.md#建造)重建。不能把全部实例记录都视作 modifier，也不因重置另建卡牌身份；跨玩家转区的归属变化另按[6.7](#控制权变化)处理。
...
**buff 与状态的关系：** buff 是有利修正的通称，例如攻击增加、获得护甲或临时获得保留；减益也是修正。耐久、历史记录、进度等状态不因具有数值就都属于 buff。后天赋予的修正依 modifier 规则清除；[永久卡面成长](#永久卡面成长)改变实例基础数值，不属于这类 modifier；原始卡面自带的能力仍属于卡牌定义，不能与“该能力产生的状态”混为一项。清除一份获得的圣盾，不等于删除卡面上能够再次赋予圣盾的能力。
...
本表索引各状态的含义和已有裁定；具体步骤只由链接的主定义维护。本轮授权补全与明确确认的来源区别记录在维护附录。

| 状态或修正 | 记录什么、怎样使用 | 转区保留类型／处理 | 消耗、到期或主定义 |
| --- | --- | --- | --- |
| 剩余耐久 | 该实例剩余耐久值。 | 跨区保留型。 | 战场实际进入弃牌堆时消耗；耗尽移除，见[6.2.5](#耐久度)。 |
| 整局累计打出次数 | 该实例本局已经打出的历史次数。 | 跨区保留型；清除 buff 不撤销历史。 | “打出”默认指从手牌执行的普通放置；效果召唤／放置不增加该历史计数。 |
...
| 永久卡面成长 | 已经直接增加到该实例卡面的基础数值。 | 跨区保留，不是 modifier；沉默不清除。 | 成长字段与实例定义见[6.2.8](#永久卡面成长)。 |
...
各类型的生成和占格要求见[2.2](elements.md#卡牌种类与分类维度)；生成须满足对应类型的限制。效果召唤／放置免除普通领土与箭头要求、数量不足尽量执行以及建造例外，统一见[4.2.5](actions.md#效果召唤与放置)；不由本节另立第二套许可。
...
**沉默与形态。** 普通变身属于可清除的形态 modifier，沉默时撤销并按撤销修正的规则截取当前生命／行动上限；明确永久变身已更新形态还原基准，沉默不撤销该形态，但会清除其效果与可清除状态。既有永久卡面成长同样保留。
...
**统一当前归属（已确认）。** 拥有者与控制者合并为当前归属玩家；卡文中的“你／己方”以该玩家为参照。创建者只记录历史来源，不保留另一份操作权或收益权。

**方向与实例记录（已确认）。** 在战场上转交控制权后，箭头及前方／上方按新控制者的朝向解释，友敌关系与持续作用随之重新计算。转交本身不改变卡牌位置、职业、伤势、buff、进度、剩余行动点或已用次数，不视为重新入场，不刷新额度。持续作用实际增加或撤销时，仍应用相应属性变化规则。

已经触发的效果所属玩家不随本体转交，受益与选择统一见[8.7.2](effects.md#来源离场封锁与控制权变化)。

### 6.7.2 控制改变后的拾取与离场

仅在原格改变控制权，没有发生[拾取词条](../reference/keywords.md#拾取)要求的进入行为，不自动补一次拾取。

**转区与接收玩家（已确认）。** 卡牌默认离场进入当前归属玩家的对应区域；效果明确写出接收玩家时按卡文处理。进入另一玩家的手牌、牌堆、弃牌堆、额外卡组、商店或虚空时，归属同步交给该玩家。区域转移的正逆向及状态处理仍依[6.5](#区域转移与状态保留)。资源拾取后直接消失，不套用普通离场目的地。

「飞船失火！」购买后进入对方弃牌堆并归对方所有；它后来留在该玩家手牌时，伤害该玩家的己方单位。「敌后渗透者」交给对方后，死亡进入对方弃牌堆，少抽一张的惩罚由死亡时的控制者承担。未来若设计临时控制，卡文须明确归还时点与接收玩家，不能靠创建者记录推断。
...
**开始拾取时重检（已确认）。** 真正开始拾取时，进入资源格的卡牌必须仍在原格、仍受玩家控制，且该资源仍存在。进入者在入场效果或湮灭结算中离场、移走，便不再执行该次拾取，资源留下；双方进入者都湮灭时同样如此。若进入者期间改变控制权，由开始拾取时的控制者取得本次拾取的玩家收益。这个检查不要求资源收益一定能够生效，收益失败仍消耗资源，见[6.8.2](#拾取与收益有效性)。

**拾取默认属于慢速动作（已确认）。** 时动阶段因移动进入资源格时当场结算拾取；时停部署中只形成待处理的拾取机会，不即时结算资源收益；统一在[3.3.1](rounds.md#时停结束与湮灭)的“湮灭结算 → 拾取结算 → 慢速效果结算”中处理。不能预支尚未结算的拾取收益，也不能将拾取混排到普通慢速之后。其他阶段由效果位移或放置引发的拾取，依[慢速通则](../reference/keywords.md#慢速)当场处理，不另等下一次时停末。
...
**拾取不以收益能够生效为条件（已确认）。** 玩家控制的卡牌进入资源格，按规定时点处理拾取；资源效果能否执行另行判断。拾取的随从接受生命、护甲、攻击、速度等卡牌收益，星能等玩家收益交给拾取玩家。法术也会拾取并消耗资源，没有对应属性时，相应收益不执行；不能因为收益无效而留下资源或撤销拾取。可执行的其他部分依通用结算规则继续处理。

「护甲碎片」「能量膜」使拾取的随从获得1点护甲。「美味的野果」使拾取的随从恢复2点生命、本回合速度＋2；剩余行动点依[6.2.2](#速度与剩余行动点)等量增加，回合末撤销速度加成，不写成恢复2行动点。

**一次性处理（试玩暂定）。** 一次拾取开始后，该资源不再接受第二次拾取；处理本次收益后本体消失，即使收益全部未生效也消失。这是一次性资源的结算保护，不新增收益合法性检查。效果已触发后的所属玩家按[8.7.2](effects.md#来源离场封锁与控制权变化)保持不变。

拾取开始前的进入者与资源重检，统一按[6.8.1](#拾取完成流程)处理。
...
「高等阳炎术」令放出的法术获得相同保留回合数时，复制本卡的**实际保留回合数**作为目标的实际计数，不复制已含光环的有效读值，也不是额外执行多次保留。该赋值不产生“被保留时”触发。随后按各目标的有效修正重新计算读值：本卡实际2、圣女视作+2为4，目标继承实际2，仍受圣女时最终同为4，不能重复把光环加成继承为6。
