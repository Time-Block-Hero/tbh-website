...
手牌与场上法术采用同一清理快照，保留判断和后续触发顺序统一见[3.6](rounds.md#回合结束)，不逐张清理并插入普通触发。
...
**待补充的具体边界：** 时动移动当场拾取已确认；其他自动阶段产生的拾取仍须明示时点。湮灭后原进入者已离场、移位或变更归属时怎样检查；拾取开始后如何固定受益玩家及防止重复拾取。这些条件会改变结果，仍见[资源牌剩余边界](../maintenance/pending.md#资源牌剩余边界)，不把上一轮未获确认的提案当作规则。资源不可堆叠，不提供同格多资源的逐个拾取规则。
