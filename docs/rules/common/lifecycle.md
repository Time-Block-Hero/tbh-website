...
5. 剩余伤害扣除目标当前生命。完成对应的单次变化或同时批次后，依第七章处理必要检查及后续效果。
...
回合开始的一批抽牌按[3.2](rounds.md#回合开始)先完成该批再处理抽牌引发的效果。其他效果写“抽 N 张”还是“重复 N 次抽一张”时，其批次边界与触发插入点需要按第七章说明，不能默认为完全相同。
...
图 6-B 不决定资源拾取与其他入场效果谁先处理；这一交互见[7.5.3](effects.md#放置入场与拾取的衔接)。同一资源在其收益结算期间能否被另一个进入事件再次拾取、拾取者中途离场或改变控制者时怎样处理，列为 **LIF-R06**；本稿不擅自增加重复收益，也不提前把资源移到虚空来规避此问题。
