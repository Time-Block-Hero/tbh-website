...
普通“受到伤害／造成伤害”触发是否要求实际扣除生命，以及护甲完全抵挡与其他零数值情形的触发语义，仍跟进 LIF-R03。旧稿默认仅实际扣血才触发，尚未获批；不能把该草案当成取消已发生伤害事件的理由。
...
属性修正导致的当前生命变化不等同于治疗或伤害，不自动产生对应的受伤／治疗事件。生命为负数时如何记录、以及变身切换基础属性时与当前值怎样联动，仍须另外明确；本条不把所有设定上限或变身效果等同 buff／debuff。
...
例如当前速度 5、剩余行动点 3，受到 −2 速度修正后成为速度 3、剩余行动点 1。回合开始的刷新仍依[3.2](rounds.md#回合开始)，一次加速不自动补满整回合额度。减速幅度超过剩余行动点时是否截断到 0、之后撤销减速怎样恢复，是下限与恢复的具体边界，跟进 LIF-R01。
...
| 后天获得的圣盾 | 有／无两态；重复获得不叠加抵挡次数。 | 逆向重置型，清除获得的状态。 | 抵挡后消除；原生圣盾重入的重新建立仍待定，主定义见[圣盾](../reference/keywords.md#圣盾)。 |
...
召唤或生成可能满足[入场](../reference/keywords.md#入场)，但不自动计为玩家普通放置。新入场随从当回合可以在己方时动行动，见[4.1.2](actions.md#新入场随从的行动)；具体部署限制、初始额度与离场重入的刷新仍按对应规则，不额外增加等待下一回合的限制。
