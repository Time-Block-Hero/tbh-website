...
**实例记录可以有不同的保留期限。** 剩余耐久和“一张牌在本局累计被打出几次”等跨卡组循环记录，不随逆向转区清除 modifier 而重置。统计期间为本回合的能力额度仍按自己的刷新规则处理；不能把所有状态一并清空或一并永久保留。主动能力默认见[4.5](actions.md#主动能力)，记录的设计维度见[7.6.8](effect-design/dictionary/usage.md)。
...
哪些区域允许累计、离场或变形后是否保留进度，以及外部直接触发已完成一次性任务的许可，须由具体进度机制明确，当前边界跟进 **LIF-R10**。不能因为耐久跨循环保留就自动规定所有进度永久保留。建造仍由[4.2.3](actions.md#建造)规定，不自动采用这里的循环扣除机制。
