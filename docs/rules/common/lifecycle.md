...
5. 剩余伤害扣除目标当前生命。先结算受伤引发的效果，再检查是否死亡，按[6.3.1](#确认死亡离场与死亡效果)处理；同批与嵌套效果的细分边界见第八章。
...
**尚需裁定：** 目标剩余生命为 2 而穿透防护的伤害为 5 时，供“受到多少伤害”引用的量算 5 还是 2；本稿不把超额伤害自动截断。受伤触发先于死亡检查的规则见[6.3.1](#确认死亡离场与死亡效果)；同时伤害的批次边界和嵌套触发顺序仍见 LIF-R03／T-01。
...
- 到死亡检查时，当前生命值为 **0 或以下**。伤害引发的检查须先完成下节规定的受伤效果结算。
- 有效效果明确要求**消灭**该随从。
...
**LIF-014 先结算受伤效果，再检查死亡（已确认）。** 伤害扣除生命后，先结算这次受伤引发的效果，再检查当前生命。即使扣血后暂时为 0 或以下，也不先清理本体而使其失去本次受伤效果的结算机会；若效果将生命恢复到正数，该随从存活，不产生本次死亡或死亡效果。

例如，假设随从当前生命为 1、生命上限至少为 2，具有“受到伤害后，回复自身 2 点生命”：受到 1 点伤害后，生命先变为 0，再由自身效果回复到 2，随后死亡检查通过。多目标同时伤害、受伤效果引发新伤害时如何划分结算批次与检查点，仍由 T-01 细化。本条不将直接消灭、生命成本或上限变化视作伤害。
...
| 发生的原因 | 是否属于死亡 | 对应处理 |
| --- | --- | --- |
| 到死亡检查时，当前生命仍为 0 或以下 | 是 | 伤害先按 6.3.1 结算受伤效果；确认死亡后清理本体，再结算死亡效果。 |
