...
**数量不足尽量执行（已确认）。** “召唤N张”默认按可用牌源和合法落点尽量执行，不要求必须凑齐N张。麦哲伦只有一张可用佣兵或一个合法指定格时召唤一张；没有可用牌或位置时不召唤。此规则不放宽明确“选择两个目标”的选择数量，也不允许少付成本。多张牌先全部完成本批入场，再结算后续，见[8.5.4批量结算通则](effects.md#批量结算通则)。牌与格的选择／分配方式依卡文，尚未明确项见[A-14](../maintenance/pending.md#第四章审阅事项)。
