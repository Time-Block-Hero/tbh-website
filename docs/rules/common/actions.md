...
**第四章修订稿。** 操作窗口、明示放置成本、三种移动方式、拾取资格、攻击目标及主动能力默认次数已按本轮讨论更新。反击、入场效果顺序等剩余问题见[第四章审阅事项](../maintenance/pending.md#第四章审阅事项)。
...
行动产生后，处理本次应完成的触发、状态变化和其他后续事项，全部完成后才能开始本方下一次普通行动。触发排序与失败细节统一见[第七章](effects.md)；另一方是否可同时操作依本章[操作窗口](#操作窗口)判断。
...
5. 卡牌离开手牌进入战场。其位置与控制关系确定；箭头、持续效果及限制从满足相应生效条件时开始适用。建造中的随从遵守[可建造](../reference/keywords.md#可建造)的封锁，不能提供卡面箭头或发动已封锁的效果。
...
**尚需决定的入场时序：** 放置自身的选择与成本顺序已经确认；以下三项会改变实际结果，需另行讨论，不能以步骤 6 的列举顺序当作答案：

- **入场效果的目标何时选？** 是放置提交前选定，还是卡牌已经进入战场、开始处理该入场效果时选择？
- **同时满足拾取与入场时，谁先结算？** 例如资源提供星能、入场效果消耗星能，顺序会影响效果能否完成。持续修正何时生效也须落在这一流程中。
- **支付成本改变了位置条件怎么办？** 例如支付成本移走唯一提供箭头的随从，是仍完成已经合法提交的放置，还是重新检查？后者还须规定支付后无法入场的处理。

这些问题记录为 [A-02](../maintenance/pending.md#第四章审阅事项)；第七章统一定义效果链的插入与返回方式。

### 4.2.2 放置与拾取

玩家控制的卡牌被放置到资源所在格时，按[6.8 拾取](lifecycle.md#拾取)为该牌的控制者结算收益，拾取完毕后资源本体消失。
...
玩家控制的卡牌进入资源所在格时，按[6.8 拾取](lifecycle.md#拾取)为该牌的控制者结算收益；无控制者的卡牌不触发拾取。

普通移动与冲撞逐格判断进入效果，瞬移只判断终点。拾取随移动结算，无须另行声明或额外支付一次移动成本。同格多资源或多个进入效果的先后由第七章统一规定。

## 4.4 攻击与反击

**ACT-005 普通攻击条件。** 在自己的时动阶段，选择仍在场、由自己控制、尚有攻击次数且允许攻击的随从。本稿普通攻击范围为上下左右相邻格，攻击只能以战场上的**非己方随从**为目标，即不由当前攻击玩家控制的随从；特殊攻击范围由具体效果规定。
...
5. 本批伤害全部处理后，再处理死亡及应在此时发生的后续效果；随后处理攻击后事项。
6. 本次后续处理全部完成后，攻击结束。
...
没有另行说明时按每回合一次计算；能力明确给出其他次数或统计范围时，按说明处理，例如每回合两次、每次入场或整局。回合开始只刷新按回合计算的限次；其他范围不自动恢复。若具体能力改用“尝试发动次数”等其他口径，须明确说明。
