...
图 4-B 只规定入场效果与拾取的相对顺序。明示延迟的效果仍在其指定时点执行，不因本图提前；入场中途离场、移位或资源消失后的拾取重检仍待明确；多个入场与嵌套效果按[8.5.3](effects.md#放置入场与拾取的衔接)细化。
...
**建造值与箭头。** 未固定值等于每个时刻指向建筑所在格的有效箭头数；当前建造值等于已固定值加未固定值。箭头移走或失效时，未固定值随之减少；反复移入移出同一箭头不会累计额外进度。每回合结束时，将当时的未固定值加到本回合已固定值，记录为下一回合的已固定值；下一回合开始时再以该起点和当时箭头计算当前值。同一组箭头持续支持，可以逐回合继续贡献。
