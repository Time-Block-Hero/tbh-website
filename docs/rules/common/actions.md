...
**ACT-009 默认入场速度与明示延迟。** 独立时停的入场效果默认慢速，只有标明快速的正文当场执行；准备选择遵守[8.2.1](effects.md#选择时点与操作窗口)。其他触发效果按卡文明示的时机处理，不能将所有时停效果一概延迟。时停内产生的慢速按[慢速词条](../reference/keywords.md#慢速)引用的结束流程处理，也不因此开放时停主动能力窗口。
