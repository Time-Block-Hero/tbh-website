...
**海燕的穿越中断。** 「海燕」可以穿过卡牌阻挡，但选择的终点仍必须是合法落脚格。如果它经过占用格后仍在场，却因后续效果等原因无法继续路线，优先回退到此前路径上最近、且当前仍可合法停留的格子。若此前路径上的格子都不可用，允许离开原路线，将位置修正到距离中断位置最近的合法空格；同距时沿用棋盘固定方向“上→右→下→左”的确定顺序。该位置修正不额外消耗行动点，已经消耗的行动点与已经结算的效果不撤回。海燕已经离场或被其他效果明确移到别处时，保留该结果，不再执行此回退。
