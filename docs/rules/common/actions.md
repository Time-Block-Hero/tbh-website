...
**第四章修订稿。** 操作窗口、明示放置成本、三种移动方式、拾取资格、攻击目标及主动能力默认次数已按本轮讨论更新。本轮已确认反击、入场选择与建造；其余审阅事项见[第四章审阅事项](../maintenance/pending.md#第四章审阅事项)。
...
6. 记录一次本次时停成功放置，先结算[入场](../reference/keywords.md#入场)效果；时停拾取登记后留到阶段结束处理，具体衔接见 [4.2.2](#放置与拾取)。
...
图 4-B 展示时停放置；虚线表示之后的阶段结算，不表示本次放置要等待双方结束时停才能完成。明示延迟的效果仍在其指定时点执行，不因本图提前；入场中途离场、移位或资源消失后的拾取重检仍待明确；多个入场与嵌套效果按[8.5.3](effects.md#放置入场与拾取的衔接)细化。
...
**本卡效果已确认。** 「谕晶」主动能力从自己的牌堆随机提供三张候选，由玩家选择其中一张，无视通常放置规则，将选中的那张牌放到谕晶的上方格子，并触发适用的入场效果。可建造牌自身入场仍依[4.2.3](#建造)在完成建造时触发；不能用本例撤销已经确认的建造特例。

这是选中牌从牌堆到战场的实际转区，不是复制新牌，也不是先抽到手牌再打出。方向依所属玩家的上方理解；其特殊许可来自卡文明示，不自动推广给所有召唤或效果放置。

候选不足三张、未选牌顺序以及上方格已占用／越界时的处理仍列为具体边界；不能擅自用默认三选一规则补出未确认处理。当前确认不对玩家额外征收选中牌的购买标价。
...
3. 攻击前响应完成后，重新检查双方仍在场、目标仍为非己方随从，以及双方各自的攻击范围。攻击自身不再合法或被取消时，不进行战斗伤害，已消耗次数不返还；仅反击条件不满足时，只取消反击。
...
次数与已支付成本都不因反制返还。多项成本整体支付、普通触发等待主体完成，见[8.3](effects.md#成本支付)；准备时主动取消或支付前核验失败，不消耗次数。使用额度已消耗不等于效果成功发动或主体成功执行，其他读取“成功发动”的条件须按自己的事件定义判断。
