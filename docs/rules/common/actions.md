...
延迟效果须说明锁定的对象是什么。例如，“结算时该格上的单位”与“此前指定的那次入场单位”可能得到不同结果。指定战场目标离场后失效、重新入场也不恢复，统一见[8.2.3](effects.md#无合法选项与目标失效)；改变控制者或位置后的其他资格仍按该效果的目标条件检查，尚未裁定的交互不能由玩家自行选择有利解释。
