...
6. 记录本次普通手牌放置，按[3.4.1](rounds.md#时动行动顺序)计入实际次数及排序计数，并触发[入场](../reference/keywords.md#入场)效果：快速效果当场执行，默认入场效果完成准备后登记到时停结束队列。时停拾取同样登记后留到阶段结束，具体衔接见[4.2.2](#放置与拾取)。
...
**入场触发与准备先于拾取，正文按速度处理。** 独立时停中的快速入场正文当场执行；没有快速标记的入场正文登记到时停结束队列。时停末依次结算湮灭、拾取、慢速正文；完整拾取流程见[6.8](lifecycle.md#拾取)。因此，准备入场效果时若需支付星能，不能预先使用尚未拾取的收益；默认慢速正文真正执行时，拾取步骤已经完成。时停外的入场正文仍先于此次拾取执行。
...
图 4-B 展示时停放置；入场节点包含快速正文或慢速目标准备／登记。虚线表示之后的阶段结算，本次放置不必等待双方结束时停才能完成。明示延迟的效果仍在其指定时点执行，不因本图提前；拾取开始前的资格变化按[6.8.1](lifecycle.md#拾取完成流程)重检；多个入场与嵌套效果按[8.5.3](effects.md#放置入场与拾取的衔接)细化。
...
**海燕的穿越中断。** 「海燕」可以穿过卡牌阻挡，但选择的终点仍必须是合法落脚格。如果它经过占用格后仍在场，却因后续效果等原因无法继续路线，优先回退到此前路径上最近、且当前仍可合法停留的格子。若此前路径上的格子都不可用，允许离开原路线，将位置修正到距离中断位置最近的合法空格；同距时沿用棋盘固定方向“上→右→下→左”的确定顺序。该位置修正不额外消耗行动点，已经消耗的行动点与已经结算的效果不撤回。海燕已经离场或被其他效果明确移到别处时，保留该结果，不再执行此回退。此次回退或后备落点属于位置修正，不是新的一段移动，不额外产生移动格数进度；只在最终落点按通用规则检查资源拾取，资源的合法共存不视为阻挡。
...
瞬移无视沿途碰撞，不经过中间格，不拾取途中资源，也不触发途中格的进入效果。终点仍须满足占格及具体效果规定的条件，例如只允许瞬移到具有某种特征的格子；无视途中碰撞不取消终点合法性检查。到达终点后处理该格的进入效果与拾取。「疯狂的传送师-贾格」要求的空格可包含资源，但不能含随从或法术，按[2.5的占格定义](elements.md#棋盘与空间关系)判断；瞬移到资源格仍执行正常拾取。这项主动效果不因此变成支付1行动点的瞬移行动，费用仍依[主动能力](#主动能力)及其自身文本。
...
**幼崽噗噜兽的“冲撞移动”（已确认）。** 取代普通选终点移动；选择上下左右方向后，逐格移动至行动点耗尽、遇阻或棋盘边界，每实际走1格消耗1行动点。没有另设格数上限，速度变化后的剩余行动点照常适用。失去此效果后恢复通用移动方式。
...
**ACT-009 默认触发速度与明示时序。** 独立时停中的普通卡牌触发效果默认慢速，入场、移除、被保留等都适用；只有标明快速的正文当场执行，准备选择遵守[8.2.1](effects.md#选择时点与操作窗口)。发生前响应、替换、持续贡献及明确的专门时序按[3.3.2](rounds.md#待结算效果的登记与顺序)处理，不随普通触发延后。默认慢速也不开放时停主动能力窗口。
