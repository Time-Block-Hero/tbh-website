...
本条只决定入场当回合的行动许可。离场再入场是否重置已经消耗的行动点、攻击次数或能力次数，另行裁定，不能用“可以行动”推导出免费刷新。
...
次数与已支付成本都不因反制返还。完整支付中的多个成本如何插入触发等剩余细节仍见 T-04；准备时主动取消或支付前核验失败，不消耗次数。使用额度已消耗不等于效果成功发动或主体成功执行，其他读取“成功发动”的条件须按自己的事件定义判断。
