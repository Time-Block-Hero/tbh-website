...
本章是标准对战的审阅草案。构筑数量、费用和稀有度限制取自两版旧规则书；地图领土的具体划分另以固定版本的实现作为提案依据，见[来源记录](../maintenance/sources.md#标准对战三章起草)。
...
**STD-007 标准双人棋盘为纵向 16 行、横向 8 列。** 本章以固定俯视方向从上到下编号第 1–16 行、从左到右编号第 1–8 列；编号不随玩家视角翻转。卡文中的前方、上方和箭头朝向仍按[空间关系](../common/elements.md#棋盘与空间关系)解释。

**以下领土划分为待确认地图提案（STD-Q1）：** 采用当前参考对局的两块 6×6 领土；A 玩家从下方向上，B 玩家从上方向下。

| 区域 | 固定棋盘坐标 |
| --- | --- |
| A 玩家领土 | 第 10–15 行，第 2–7 列 |
| B 玩家领土 | 第 2–7 行，第 2–7 列 |
| 无玩家领土归属的区域 | 外围一圈，以及中央第 8–9 行 |

领土属于格子，不因随从移动经过或占据它就自动改变。本章所称领土与卡牌当前归属分别判断；领土影响普通放置与资源生成，具体行动合法性仍见[第四章](../common/actions.md)。

### 10.3.2 英雄部署

**STD-008 玩家在己方领土最后一排的合法空格中选择英雄起点。** 按上述地图提案，A 可选择第 15 行第 2–7 列，B 可选择第 2 行第 2–7 列。

部署作为开局配置完成，不消耗手牌、星能或箭头，也不计入任何回合的普通放置次数。英雄按卡面建立当前生命与行动额度，其生效范围内的持续效果随开局配置生效；开局部署不额外发动一次入场效果。

本稿采用双方同时秘密选择、双方确认后一起公开并完成部署。英雄初始部署完成前，不开放时停或时动操作窗口。部署选择与初始化的这两项处理属于本轮补全的开局默认值，等待随本章审阅。
...
2. 建立本局棋盘和领土，按模式启用的配置建立初始地图对象；没有已启用的配置，不自行补造对象。
...
- [模式概览与胜负](overview.md)
- [对局自动逻辑](automatic.md)
- [待决定的地图及英雄边界](../maintenance/pending.md#第二部分待裁定)
