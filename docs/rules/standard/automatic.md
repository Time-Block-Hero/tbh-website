...
本章是标准对战的审阅草案，定义玩家操作之外的模式自动变化。当前补全的是“遗失的智慧”生成；野怪与其他地图事件没有完整配置，集中见[STD-Q3](../maintenance/pending.md#第二部分待裁定)。通用回合、卡牌状态与效果本身仍引用各自主定义。
...
本版没有把“野怪”“天气”或某个地图名字视作自带一套未写明的系统行动。野怪可以作为卡牌存在，其效果依卡文；是否开局生成、之后何时刷新和自动行动，必须另有本模式启用的配置。当前旧资料只留下概念，尚不足以补成唯一可执行的标准野怪配置，见[STD-Q3](../maintenance/pending.md#第二部分待裁定)。
...
- [第二部分待裁定](../maintenance/pending.md#第二部分待裁定)
