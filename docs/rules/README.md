...
这里是规则 Wiki 的 Markdown 源码。五组导航、十二章的目录与范围已获用户确认；所有页面仍为 `draft`，正文待逐主题裁定。目录标题与提纲不构成已确认玩法。不要自动复制旧 Rulebook 或按当前代码补写规则。
...
每次选一个主题，比较旧文档和实现，列出冲突与缺口，以具体局面的问答逐项确认。用户确认后再写入规则、边界、案例和关联页面，并记录替代了哪些旧章节；工程实现与设计决定分别记录。
