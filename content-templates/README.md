...
- `bestiary-taxonomy.js` owns the six race headings and concrete branch identities. `bestiary-data.js` retains the complete existing 15 source dossiers, with umbrella dossiers used as race-wide context. `bestiary-card-sources.js` is a generated, non-authoritative display snapshot of associated cards.

The archive is the visual production reference for consistent game assets. Follow [the setting art contract](../references/setting-art-contract.md): race constraints → branch constraints → individual references. Distinguish approved invariants, examples, allowed variation and incomplete decisions.

Run `npm run build` after card changes or setting-map changes to refresh both setting and bestiary card-reference snapshots. It generates `setting-data.js` for static/offline display, including names, all mapped forms, the current selected artwork, and art descriptions. Do not hand-edit the generated file. The website should also resolve these mappings against current card data when available, so editor changes can be reflected without maintaining a second card database.
